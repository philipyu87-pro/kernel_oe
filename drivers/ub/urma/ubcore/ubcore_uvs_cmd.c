...
static int ubcore_create_jetty_rsrc(struct ubcore_topo_map *topo_map)
{
    ...
    for (dev_idx = 0; dev_idx < DEV_NUM; dev_idx++) {
        for (die_idx = 0; die_idx < IODIE_NUM; die_idx++) {
            if (!is_eid_valid(cur_node_info->agg_devs[dev_idx].ues[die_idx].primary_eid))
                continue;
            ...
            if (dev == NULL) {
                ubcore_log_err("primary dev not exist, node %d dev %d die %d, eid: " EID_FMT "\n",
                        ...
                        EID_RAW_ARGS(cur_node_info->agg_devs[dev_idx].ues[die_idx].primary_eid));
                return -1;
            }
			       ...
			       struct ubcore_cmd_hdr *hdr)
{
	...
	if (topo_map == NULL) {
		...
		if (!is_agg_and_primary_eid_valid(topo_map)) {
