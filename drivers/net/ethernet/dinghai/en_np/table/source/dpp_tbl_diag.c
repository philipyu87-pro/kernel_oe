...
ZXIC_UINT32 diag_dpp_vport_table_prt(ZXIC_UINT16 slot, ZXIC_UINT16 vport)
{
    ...
    ZXIC_COMM_PRINT("%02u np_egress_meter_enable = %u\n", SRIOV_VPORT_NP_INGRESS_METER_EN_OFF, port_table.np_egress_meter_enable);
    ZXIC_COMM_PRINT("%02u np_ingress_meter_enable = %u\n", SRIOV_VPORT_NP_EGRESS_METER_EN_OFF, port_table.np_ingress_meter_enable);
...
ZXIC_UINT32 diag_dpp_rxfh_prt(ZXIC_UINT16 slot, ZXIC_UINT16 vport)
{
    ...
    for (group_id = 0; group_id < RSS_TO_VQID_GROUP_NUM; group_id++)
    {
        ZXIC_COMM_PRINT("[%s] qid%u: 0x%04x qid%u: 0x%04x qid%u: 0x%04x qid%u: 0x%04x qid%u: 0x%04x qid%u: 0x%04x qid%u: 0x%04x qid%u: 0x%04x\n",
                                                                                __FUNCTION__, (group_id * 8) + 0, queue_list[(group_id * 8) + 0],
                                    ...
                                    ZXIC_UINT32 rd_mode)
{
    ...
    ZXIC_COMM_PRINT("[%s] slot: %u vport: 0x%04x stat_item_no: %u index: %u h64_cnt: %llu l64_cnt: %llu success.\n", __FUNCTION__,
