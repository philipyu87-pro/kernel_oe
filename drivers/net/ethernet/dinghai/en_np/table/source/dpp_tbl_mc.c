...
ZXIC_UINT32 dpp_vport_mc_info_add(DPP_PF_INFO_T* pf_info, ZXIC_CONST ZXIC_VOID *mac, DPP_VPORT_MC_INFO_T *mc_info)
{
    ...
    ZXIC_COMM_TRACE_NOTICE("[%s] mc_index:%u mc_bitmap:0x%016llx\n", __FUNCTION__,mc_index, mc_table->mc_info[mc_index].mc_bitmap[group_id]);
                                     ...
                                     ZXIC_CONST ZXIC_VOID *mac, DPP_VPORT_MC_INFO_T *mc_info)
{
    ...
    ZXIC_COMM_TRACE_NOTICE("[%s]vfunc_num:%u,group_id:%u,new_vfunc_num:%u,new_group_id:%u\n",
                             __FUNCTION__, vfunc_num,group_id,new_vfunc_num,new_group_id);
    ...
    for (index = 0; index < MC_TABLE_SIZE; index++)
    {
            ...
            (ZXIC_COMM_MEMCMP(mc_table->mc_info[index].mac, mac, 6) == 0))
        {
            ...
            ZXIC_COMM_TRACE_NOTICE("[%s] update success !",__FUNCTION__);
            return DPP_OK;
        }
    }

    ZXIC_COMM_TRACE_ERROR("[%s] update fail !",__FUNCTION__);
    return DPP_ERR;
}
    ...
    for (index = 0; index < MC_TABLE_SIZE; index++)
    {
        ...
        for (group_id = 0; group_id < MC_GROUP_NUM; group_id++)
        {
            ...
            ZXIC_COMM_TRACE_NOTICE("[%s] mac: %02x:%02x:%02x:%02x:%02x:%02x.\n", __FUNCTION__,
                                                                ...
                                                                mc_entry.key.mc_mac[2], mc_entry.key.mc_mac[3],
                                                                mc_entry.key.mc_mac[4], mc_entry.key.mc_mac[5]);
            ZXIC_COMM_TRACE_NOTICE("[%s] mc_bitmap: %02x %02x %02x %02x %02x %02x %02x %02x.\n", __FUNCTION__,
...
ZXIC_UINT32 dpp_batch_add_multicast_mac(DPP_PF_INFO_T* pf_info, ZXIC_UINT32 mac_num, ZXIC_CONST ZXIC_VOID *mac)
{
    ...
    for(entry_index = 0;entry_index<mac_num;entry_index++)
    {
        ...
        for(group_index=0;group_index<MC_GROUP_NUM;group_index++)
        {
            ...
            ZXIC_COMM_TRACE_NOTICE("[%s] slot: %u vport: 0x%04x group_id: %u mac: %02x:%02x:%02x:%02x:%02x:%02x bitmap:0x%016llx start.\n",
                                     __FUNCTION__,pf_info->slot, pf_info->vport,p_one_entry->key.group_id,
...
ZXIC_UINT32 dpp_batch_del_multicast_mac(DPP_PF_INFO_T* pf_info, ZXIC_UINT32 mac_num, ZXIC_CONST ZXIC_VOID *mac)
{
    ...
    for(entry_index = 0;entry_index<mac_num;entry_index++)
    {
        ...
        ZXIC_COMM_CHECK_RC_MEMORY_FREE_UNLOCK_NO_ASSERT(rc, "dpp_vport_mc_info_del", p_batch_entry, DEV_PCIE_LOCK(&dev));
        ...
        for(group_index=0;group_index<MC_GROUP_NUM;group_index++)
        {
            ...
            p_one_entry->key.group_id = group_index;
            ZXIC_COMM_MEMCPY_S(p_one_entry->key.mc_mac, 6, p_mac, 6);
            ZXIC_COMM_TRACE_NOTICE("[%s] slot: %u vport: 0x%04x group_id: %u mac: %02x:%02x:%02x:%02x:%02x:%02x start.\n",
                                     __FUNCTION__,pf_info->slot, pf_info->vport,p_one_entry->key.group_id,
                                    p_one_entry->key.mc_mac[0], p_one_entry->key.mc_mac[1],
                                    p_one_entry->key.mc_mac[2], p_one_entry->key.mc_mac[3],
                                    p_one_entry->key.mc_mac[4], p_one_entry->key.mc_mac[5]);
    ...
    rc = dpp_apt_dtb_multi_hash_delete(&dev, queue, sdt_no, MC_GROUP_NUM * mac_num, sizeof(ZXDH_MC_T),p_batch_entry);
    ZXIC_COMM_FREE(p_batch_entry);
...
ZXIC_UINT32 dpp_multicast_mac_transfer_intra_group(DPP_DEV_T *dev, ZXIC_UINT32 slot, ZXIC_UINT32 vport, ZXIC_UINT32 new_vport)
{
    ...
    for(index = 0; index < entryNum; index++)
    {
        ...
        if(TRUE==dpp_vport_in_mc_bitmap(vport,p_mc_temp_entry->entry.mc_bitmap))
        {
            ...
            ZXIC_COMM_TRACE_NOTICE("[%s] slot: %u vport: 0x%04x new_vport: 0x%04x mac: %02x:%02x:%02x:%02x:%02x:%02x start.\n",
                                     __FUNCTION__,slot,vport,new_vport,
            ...
            ZXIC_COMM_TRACE_NOTICE("[%s] group_id: %u mc_bitmap: 0x%016llx new_group_id: %u new_mc_bitmap: 0x%016llx\n", __FUNCTION__,
...
ZXIC_UINT32 dpp_multicast_mac_transfer(DPP_PF_INFO_T* pf_info, DPP_PF_INFO_T* new_pf_info)
{
    ...
    ZXIC_COMM_TRACE_NOTICE("[%s] slot: %u vport: 0x%04x new_vport: 0x%04x success.\n",
                            __FUNCTION__, pf_info->slot, pf_info->vport, new_pf_info->vport);
...
ZXIC_UINT32 dpp_multicast_mac_max_get(DPP_PF_INFO_T* pf_info, ZXIC_UINT32 *max_num)
{
    ...
    ZXIC_COMM_TRACE_NOTICE("[%s] slot: %u vport: 0x%04x max_num: 0x%x get succ.\n",
                           __FUNCTION__, pf_info->slot, pf_info->vport, *max_num);
    return DPP_OK;
}
