...
typedef struct dpp_stat_item_t{
    //ZXIC_UINT32 index;            /*统计项索引*/
    ZXIC_UINT32 mode;             /*统计项模式：0：64bit，1：128bit*/
    ZXIC_UINT32 addr_offset;      /*统计项地址偏移，单位与模式一致*/
    ZXIC_UINT32 depth;            /*统计项深度,单位与模式一致*/
}DPP_APT_STAT_ITEM_T;
#if 0
typedef struct dpp_stat_tbl_init_t
{
    ZXIC_UINT32 stat_item_num;                              /*统计项个数,最多256个*/
    DPP_APT_STAT_ITEM_T stat_item[STAT_ITEM_MAX_NUM];       /*统计表项信息*/
} DPP_APT_STAT_TBL_INIT_T;
#endif
