...
static int32_t zxdh_lag_hash(struct zxdh_lag *ldev)
{
    ...
    dpp_lag_group_hash_factor_set(&pf_info, ldev->lag_id, (uint8_t)np_hash_type);
...
int32_t lag_dpp_pannel_init(struct dh_core_dev *dh_dev, struct zxdh_lag_attrs *attr)
{
    ...
    LAG_LOG_INFO("vport 0x%hx, pannel id %hu, phy port %hu, rx qid %hu tx qid %hu bond vfid %d, ovs vfid %d, lag_id %d\n",
