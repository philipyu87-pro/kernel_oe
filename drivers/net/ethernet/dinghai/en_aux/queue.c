...
void zxdh_print_vring_info(struct virtqueue *vq, uint32_t desc_index, uint32_t desc_num)
{
    ...
    for (i = 0; i < desc_num; i++)
    {
        ...
        LOG_INFO("DESC[%d] 0x%llx:\t0x%016llx\t0x%08x\t%8d\t0x%x\n", \
                 j, (uint64_t)desc, desc->addr, desc->len, desc->id, desc->flags);
    }
...
void zxdh_vvq_reset(struct zxdh_en_device *en_dev)
{
    ...
    for (i = 0; i < 2 * en_dev->max_queue_pairs; ++i) {
        ...
        memset(vvq->packed.vring.desc, 0, num * sizeof(struct vring_packed_desc) + ZXDH_DESC_EXTRA_SIZE);
                              ...
                              const char *name, bool ctx, uint16_t channel_num)
{
    ...
    err = en_dev->ops->vq_bind_channel(en_dev->parent, channel_num, en_dev->phy_index[index], index);
    if (err < 0)
    {
...
/* Memory accessors */
uint16_t vqm16_to_cpu(struct zxdh_en_device *en_dev, __vqm16 val)
{
...
void pd_receive_handle(struct net_device *netdev, struct sk_buff *skb, struct zxdh_net_hdr_rx *hdr_rcv,  struct virtnet_rq_stats *stats)
{
    uint16_t cvid = 0;
    uint16_t svid = 0;
    uint16_t vid = 0;
...
int32_t vp_get_phy_vqs(struct net_device *netdev, uint16_t vq_cnt, uint32_t *phy_index, const char *type)
{
    ...
    if (fw_patch < DH_NEW_QUEEU_ALLOC_PATCH)
    {
        ...
        if (en_dev->ops->is_bond(en_dev->parent))
        {
            ...
            LOG_DEBUG("old_msg->reps.cmn_vq_msg.queue_nums=%u", old_msg->reps.cmn_vq_msg.queue_nums);
            for (i=0; i<old_msg->reps.cmn_vq_msg.queue_nums; i++)
            {
                LOG_DEBUG("old_msg->reps.cmn_vq_msg.phy_qidx[%u]: %u", i, old_msg->reps.cmn_vq_msg.phy_qidx[i]);
            }
        }
    }
...
err_get_lock:
    ...
    printk(KERN_CONT "[zxdh_pf][%s][%d] %s phy_index: ", __func__, __LINE__, type);
    for (i = 0; i < vq_cnt; i++)
    {
        printk(KERN_CONT "%u ", phy_index[i]);
    }
    printk(KERN_CONT "\n");
...
int8_t zxdh_sec_create_vqs(struct net_device *netdev, struct zxdh_sec_info *sec_info, uint8_t qidx)
{
    ...
    en_dev->ops->activate_phy_vq(en_dev->parent, en_dev->sec_phy_index[qidx], ZXDH_SEC_MIN_DESC_NUM,
                                 ring_dma_addr, driver_event_dma_addr, device_event_dma_addr);
    notify_addr = (void __force *)en_dev->ops->vp_modern_map_vq_notify(en_dev->parent, en_dev->sec_phy_index[qidx], &en_dev->notify_phy_addr);
...
{
    uint16_t queue_pairs = en_dev->curr_queue_pairs;
