                                            ...
                                            uint32_t field, uint16_t *bytes, uint16_t *data, union zxdh_msg *old_msg)
{
    ...
    if (OP_CODE_DATA_CHAN == field)
    {
        *bytes = (uint16_t)((queue_nums + 1) * ZXDH_QS_PAIRS);
        ...
        if (old_msg != NULL)
        {
            LOG_DEBUG("old_msg->reps.cmn_vq_msg.queue_nums: %u; queue_nums: %u", old_msg->reps.cmn_vq_msg.queue_nums, queue_nums);
            if (old_msg->reps.cmn_vq_msg.queue_nums > 0)
            {
                old_queue_nums = old_msg->reps.cmn_vq_msg.queue_nums;
                if ((old_queue_nums + queue_nums) > 256)
                {
                    LOG_ERR("Exceeded the maximum number of queues, old_queue_nums(%d)+queue_nums(%d)\n", old_queue_nums, queue_nums);
                    return -1;
                }
    ...
    else if (OP_CODE_MSGQ_CHAN == field)
    {
        ...
        *bytes = (uint16_t)(queue_nums * ZXDH_QS_PAIRS);
        data[0] = en_dev->phy_index[en_dev->curr_queue_pairs*2];    //en_dev->rq[en_dev->curr_queue_pairs].vq->phy_index;
        data[1] = en_dev->phy_index[en_dev->curr_queue_pairs*2+1];  //en_dev->sq[en_dev->curr_queue_pairs].vq->phy_index;
    }
#endif
...
int32_t zxdh_slot_info_send(struct zxdh_en_device *en_dev, uint8_t *slot_info)
{
    int32_t ret = 0;
    union zxdh_msg *msg = NULL;
    struct zxdh_bar_extra_para para = {0};
...
/* Started by AICoder, pid:f65bbz8feaa0ad114c5a0a4f7052b7297a83eb86 */
int8_t zxdh_debug_ip_get(struct zxdh_en_device *en_dev, int8_t *ip)
{
    ...
    slot_info = (uint8_t)((en_dev->slot_id) & 0xff); /* 获取槽位号的低8bit */
    slot_info++; /* 防止槽位号是1，生成ip是网关 */
    sprintf(ip_address, "26.20.5.%d", slot_info); /* 直接将slot_info转换为字符串并添加到IP地址字符串后面 */
...
int32_t zxdh_vport_stats_get(struct zxdh_en_device *en_dev)
{
    ...
    vport_stats->dtp_stats.rx_udp_csum_fail_packets -= en_dev->pre_stats.dtp_stats.rx_udp_csum_fail_packets;
    vport_stats->dtp_stats.tx_udp_csum_fail_packets -= en_dev->pre_stats.dtp_stats.tx_udp_csum_fail_packets;
    vport_stats->dtp_stats.rx_tcp_csum_fail_packets -= en_dev->pre_stats.dtp_stats.rx_tcp_csum_fail_packets;
    vport_stats->dtp_stats.tx_tcp_csum_fail_packets -= en_dev->pre_stats.dtp_stats.tx_tcp_csum_fail_packets;
    vport_stats->dtp_stats.rx_ipv4_csum_fail_packets -= en_dev->pre_stats.dtp_stats.rx_ipv4_csum_fail_packets;
    vport_stats->dtp_stats.tx_ipv4_csum_fail_packets -= en_dev->pre_stats.dtp_stats.tx_ipv4_csum_fail_packets;

    vport_stats->np_stats.rx_vport_unicast_packets -= en_dev->pre_stats.np_stats.rx_vport_unicast_packets;
    vport_stats->np_stats.tx_vport_unicast_packets -= en_dev->pre_stats.np_stats.tx_vport_unicast_packets;
    vport_stats->np_stats.rx_vport_unicast_bytes -= en_dev->pre_stats.np_stats.rx_vport_unicast_bytes;
    vport_stats->np_stats.tx_vport_unicast_bytes -= en_dev->pre_stats.np_stats.tx_vport_unicast_bytes;
    vport_stats->np_stats.rx_vport_multicast_packets -= en_dev->pre_stats.np_stats.rx_vport_multicast_packets;
    vport_stats->np_stats.tx_vport_multicast_packets -= en_dev->pre_stats.np_stats.tx_vport_multicast_packets;
    vport_stats->np_stats.rx_vport_multicast_bytes -= en_dev->pre_stats.np_stats.rx_vport_multicast_bytes;
    vport_stats->np_stats.tx_vport_multicast_bytes -= en_dev->pre_stats.np_stats.tx_vport_multicast_bytes;
    ...
    vport_stats->np_stats.rx_vport_broadcast_bytes -= en_dev->pre_stats.np_stats.rx_vport_broadcast_bytes;
    vport_stats->np_stats.tx_vport_broadcast_bytes -= en_dev->pre_stats.np_stats.tx_vport_broadcast_bytes;
    ...
    vport_stats->np_stats.tx_vport_ssvpc_packets -= en_dev->pre_stats.np_stats.tx_vport_ssvpc_packets;
    vport_stats->np_stats.rx_vport_idma_drop_packets -= en_dev->pre_stats.np_stats.rx_vport_idma_drop_packets;
    return err;
}
...
int32_t zxdh_mac_stats_get(struct zxdh_en_device *en_dev)
{
    ...
    switch (en_dev->curr_speed_modes)
    {
        ...
        {
            stats_addr = ZXDH_SPM_STATS_OFFSET + (en_dev->phy_port % 4) * sizeof(struct zxdh_en_spm_stats);
            bytes_addr = ZXDH_SPM_BYTES_OFFSET + (en_dev->phy_port % 4) * sizeof(struct zxdh_en_spm_bytes);
            break;
        }
        case BIT(SPM_SPEED_2X_100G):
        {
            stats_addr = ZXDH_SPM_STATS_OFFSET + (4 + (en_dev->phy_port % 4) / 2) * sizeof(struct zxdh_en_spm_stats);
            bytes_addr = ZXDH_SPM_BYTES_OFFSET + (4 + (en_dev->phy_port % 4) / 2) * sizeof(struct zxdh_en_spm_bytes);
            break;
        }
...
int32_t zxdh_vf_dpp_add_ipv6_mac(struct zxdh_en_device *en_dev, const uint8_t *mac_addr)
{
    ...
    {
        LOG_ERR("Add Multicast MAC Address(%pM) Failed, Beyond Max MAC Num in the whole transfer area\n", mac_addr);
    }
    kfree(msg);
...
int32_t zxdh_get_coalesce_usecs(struct zxdh_en_device *en_dev,
                                uint32_t *rx_coalesce_usecs, uint32_t *tx_coalesce_usecs)
{
    union zxdh_msg *msg = NULL;
    int32_t err = 0;
    struct zxdh_bar_extra_para para = {0};
    ...
    while (flow_num < en_dev->fs.tot_num_rules && location < ETHTOOL_FD_MAX_NUM) {
        if (en_dev->fs.ethtool_fs[location].is_used) {
            ...
            if (en_dev->ops->get_coredev_type(en_dev->parent) == DH_COREDEV_VF) {
                ...
                if (err) {
                    LOG_ERR("zxdh_cfg_fd_add_action failed, location %d", location);
                    return -EINVAL;
                }
            ...
            flow_num++;
            LOG_INFO("recover_flow_table: location is %u, orig_index is %u, new_index is %u",
