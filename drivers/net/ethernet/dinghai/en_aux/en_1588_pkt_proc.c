                        ...
                        struct zxdh_en_device *en_dev, uint8_t *ptpHdr)
{
    ...
    {
        DEBUG_1588("unknow PTP msg type!\n");
        return -EFAULT;
    }

    LOG_DEBUG("pkt_1588_proc_xmit msg_type %d\n",msg_type);
    /* 如果是事件报文，提取时间戳 */
...
int32_t pkt_delay_statistics_proc(struct sk_buff *skb, struct zxdh_net_hdr_tx *hdr, struct zxdh_en_device *en_dev)
{
    uint8_t             *pData      = NULL;
    uint8_t             ret         = 0;
    uint32_t            ts_thw      = 0;
