                                          ...
                                          uint32_t tc_id)
{
    ...
    LOG_DEBUG("dcbnl_init_ets dpp_flow_shape_set end vport%d,phy_port:%d, flow_id:%d,tc_id:%d, cir:%d, cbs:%d, db_en:%d, eir:%d,ebs:%d,err:%d \n",
...
uint32_t zxdh_dcbnl_scheduling_tree_link_tc(struct zxdh_en_priv *en_priv)
{
    ...
    /*link_level  tc_type         tc_tx_bw  c_rate                           e_rate               td_th */
        {1,  ZXDH_DCBNL_STRICT_TC,   100,   ZXDH_DCBNL_FLOW_RATE_CIR,  ZXDH_DCBNL_INITRATE_KBITPS,  ZXDH_DCBNL_FLOW_TDTH_UPF},
...
uint32_t zxdh_dcbnl_set_tc_scheduling(struct zxdh_en_priv *en_priv, uint8_t *tc_type, uint8_t *tc_tx_bw)
{
    ...
    for (i = 0; i < ZXDH_DCBNL_MAX_TRAFFIC_CLASS && flow_node != NULL; i++)
    {
        ...
        LOG_DEBUG("clean maxrate vport%d,phy_port:%d, flow_id:%d,tc_id:%d, cir:%d, cbs:%d, db_en:%d, eir:%d,ebs:%d \n",
        ...
        LOG_DEBUG("dpp_flow_shape_set begin vport%d,phy_port:%d, flow_id:%d,tc_id:%d, cir:%d, cbs:%d, db_en:%d, eir:%d,ebs:%d \n",
            en_dev->vport,en_dev->phy_port, flow_node->flow_id, tc_id, p_para.cir, p_para.cbs, p_para.db_en, p_para.eir, p_para.ebs);
        ...
        LOG_DEBUG(" tc_id:%d, tc_type:%d, c_linkid:%x, e_weight:%d, e_sp:%d ,e_linkid:%x, e_weight:%d, e_sp:%d \n",
...
uint32_t zxdh_dcbnl_set_ets_up_tc_map(struct zxdh_en_priv *en_priv, uint8_t *prio_tc)
{
    ...
    {
        tc_td_th[prio_tc[0]] = ZXDH_DCBNL_FLOW_TDTH_UPF;  //prio0对应的TC设置成UPF的TD值
    }
...
uint32_t zxdh_dcbnl_set_tc_maxrate(struct zxdh_en_priv *en_priv, uint32_t *maxrate)
{
    ...
    for (i = 0; i < ZXDH_DCBNL_MAX_TRAFFIC_CLASS && flow_node != NULL; i++)
    {
        ...
        LOG_DEBUG(" vport%d,phy_port:%d, flow_id:%d,tc_id:%d, cir:%d, cbs:%d, db_en:%d, eir:%d,ebs:%d \n",
            en_dev->vport,en_dev->phy_port, flow_node->flow_id, tc_id, p_para.cir, p_para.cbs, p_para.db_en, p_para.eir, p_para.ebs);
...
uint32_t zxdh_dcbnl_clear_flow_td_th(struct zxdh_en_priv *en_priv)
{
    ...
    for (i = 0; i < ZXDH_DCBNL_MAX_TRAFFIC_CLASS && flow_node != NULL; i++)
    {
        if (flow_node->td_th != 0)
        {
            ...
            if (err)
            {
                LOG_ERR("dcbnl_set_ets: clear_flow_td_th failed, vport: %d, flow_id:%d, err:%d\n",
                        en_dev->vport, flow_node->flow_id, err);
                return err;
            }
            flow_node->td_th = 0;
...
uint32_t zxdh_dcbnl_printk_ets_tree(struct zxdh_en_priv *en_priv)
{
    ...
    for (level = ZXDH_DCBNL_ETS_TREE_ROOT_LEVEL; level > 0; level--)
    {
        ...
        while (NULL != se_node)
        {
            ...
            LOG_DEBUG(" se_node se_link_id:0x%x, se_link_weight:%d, se_link_sp:%d, link_point:%d \n",
...
uint32_t zxdh_dcbnl_disable_debug(struct zxdh_en_priv *en_priv)
{
    ...
    if (err)
    {
        LOG_ERR(" set debug mode diable failed \n");
    }
    return err;
}
