 ...
 */
int32_t dh_irq_affinity_irqs_request_auto(struct dh_irq_pool *pool, struct dh_irq **irqs, int32_t num_irqs, int numa)
{
   ...
   // 绑定 IRQ 对到循环分配的 CPU
    for (i = 0; i < num_irqs; i += 2) 
    {
        ...
        for (pair_offset = 0; pair_offset < 2 ; pair_offset++)
        {
            ...
            if (IS_ERR_OR_NULL(irq))
            {
                LOG_ERR("irq_pool_request_irq %d failed, req_mask=%p, irq=%p", irq_idx, req_mask, irq);
                goto cleanup;
            }
            irqs[irq_idx] = irq;
