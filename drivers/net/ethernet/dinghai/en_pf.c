...
/**
 * @brief Set the rp cpl timeout mask status object
 *
 ...
 * @return int32_t          0为正常，其余异常
 */
int32_t set_rp_hp_irq_ctrl(struct pci_dev *pdev, uint32_t status)
{
    struct pci_dev *rp_dev = NULL;
    int express = 0;
    u32 data = 0;
...
int32_t zxdh_pf_set_hp_irq_ctrl_status(struct dh_core_dev *dev, uint32_t status)
{
 ...
 */
int32_t get_rp_hp_irq_ctrl_status(struct pci_dev *pdev)
{
    struct pci_dev *rp_dev = NULL;
    u32 data = 0;
    int express = 0;
...
int32_t zxdh_pf_rp_config_init(struct dh_core_dev *dev)
{
    ...
    if (pf_dev->board_type == DH_STD_E312S ||
...
int32_t dh_pf_pci_init(struct dh_core_dev *dev)
{
    ...
    pf_dev->pci_ioremap_addr[0] = (uint64_t)(uintptr_t)ioremap(pci_resource_start(dev->pdev, 0), pci_resource_len(dev->pdev, 0));
    if (pf_dev->pci_ioremap_addr[0] == 0)
    {
        ret = -1;
        LOG_ERR("ioremap(0x%llx, 0x%llx) failed\n", (unsigned long long)pci_resource_start(dev->pdev, 0), (unsigned long long)pci_resource_len(dev->pdev, 0));
        goto err_pci_save_state;
    }
...
void dh_pf_pci_close(struct dh_core_dev *dev)
{
    ...
    pf_dev = dh_core_priv(dev);
    iounmap((void *)(uintptr_t)pf_dev->pci_ioremap_addr[0]);
    pci_disable_pcie_error_reporting(dev->pdev);
    pci_release_selected_regions(dev->pdev, pci_select_bars(dev->pdev, IORESOURCE_MEM));
    pci_disable_device(dev->pdev);
...
static uint8_t zxdh_pf_wait_bar_ok(struct dh_core_dev *dh_dev)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
    uint8_t config_generation = 0;
    uint8_t i = 0;
...
uint64_t zxdh_pf_get_spec_sbdf(struct dh_core_dev *dh_dev)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
...
uint32_t zxdh_pf_get_rp_sbdf(struct dh_core_dev *dh_dev)
{
    struct pci_dev *pdev = dh_dev->pdev;
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
...
struct zxdh_np_ext_stats * zxdh_get_np_ext_stats(struct dh_core_dev *dh_dev, uint8_t panel_id)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
    ...
    err_addr = (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_NP_EXT_STATS_OFFSET + err_offset);
    disc_addr = (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_NP_EXT_STATS_OFFSET + disc_offset);
    if ((err_addr >= (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_NP_EXT_STATS_OFFSET + ZXDH_NP_EXT_STATS_SIZE)) \
     || (disc_addr >= (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_NP_EXT_STATS_OFFSET + ZXDH_NP_EXT_STATS_SIZE)))
    {
...
uint8_t zxdh_pf_get_queue_pairs(struct dh_core_dev *dh_dev)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
...
int32_t zxdh_vf_compat_check(struct dh_core_dev *dh_dev)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
    union zxdh_msg *msg = NULL;
    int32_t err = 0;
...
int32_t dh_pf_mcode_compat_check(struct dh_core_dev *dh_dev)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
    int32_t ret = 0;
...
int32_t zxdh_pf_query_port(struct dh_core_dev *dh_dev)
{
    ...
    LOG_INFO("pcie_id: 0x%x, bond_num: %u lag_id: %u port_num: %u\n",
              payload->pcie_id, recv_data->bond_num, recv_data->bond_idx, recv_data->port_num);
...
uint16_t zxdh_pf_get_queue_vector(struct dh_core_dev *dh_dev, uint16_t channel, struct list_head * eqs_list, uint16_t queue_index, uint16_t vq_idx)
{
    struct zxdh_pf_device *pf_dev = dh_core_priv(dh_dev);
...
int32_t find_valid_vqs_by_bit(struct dh_core_dev *dh_dev, uint8_t queue_type, uint16_t vq_cnt, int32_t *phy_index, uint16_t total_qp, uint16_t start_id)
{
    ...
    {
        val = ioread32((void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + PHY_VQ_REG_OFFSET + (phy_vq_reg + phy_vq_reg_oft) * 4));
...
int32_t find_valid_vqs_by_type(struct dh_core_dev *dh_dev, uint8_t queue_type, uint16_t vq_cnt, int32_t *phy_index)
{
    ...
    for (phy_vq_reg = 0; phy_vq_reg < vq_reg_num; phy_vq_reg++)
    {
        val = ioread32((void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + PHY_VQ_REG_OFFSET + phy_vq_reg * 4));
...
int32_t zxdh_pf_find_valid_vqs(struct dh_core_dev *dh_dev, uint16_t vq_cnt, int32_t *phy_index)
{
    ...
    if ((fw_compat.patch >= 1) && zxdh_pf_is_nic(dh_dev))
    {
        ...
        dev_qinfo = (struct zxdh_dev_queue_info *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_DEV_QUEUE_INFO_OFFSET + pf_idx * 4);
        LOG_DEBUG("pf(vport:0x%x) get queue config: ep_id:%u, pf_idx:%u, total_qp:%u, start_id:%u\n",
...
int32_t zxdh_pf_write_vqs_bit(struct dh_core_dev *dh_dev, uint16_t vq_cnt, uint32_t *phy_index)
{
    ...
    for (i = 0; i < vq_cnt; ++i)
    {
        ...
        iowrite32(val, (void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + PHY_VQ_REG_OFFSET + phy_vq_reg * 4));
    }
...
int32_t zxdh_pf_write_queue_tlb(struct dh_core_dev *dh_dev, uint16_t vq_cnt, uint32_t *phy_index, bool need_msgq)
{
    ...
    for (i = 0; i < vq_cnt; ++i)
    {
        ...
        iowrite16(pcieid, (void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + pf_dev->qtlb_offset + phy_index[i] * 2));
    }
...
uint16_t zxdh_pf_get_ovs_pf_vfid(struct dh_core_dev *dh_dev)
{
    ...
    // 读取4字节的数据
    ovs_pf_vfid = ioread32((void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_OVS_PF_VFID_OFFSET));
...
static bool is_sn_invalid(uint8_t sn_code[])
{
    bool all_zero = true;
    bool all_ff = true;
    uint8_t i = 0;
...
static int32_t zxdh_nic_sn_get(struct dh_core_dev *dh_dev)
{
    ...
    memcpy(&sn_info, (void *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + DH_SN_OFFSET), sizeof(struct nic_sn_info));
    ...
    if ((sn_info.fixed_sn_valid != 0xaa) && (sn_info.pseudo_sn_valid != 0xaa)) {
        ...
        memcpy((void *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + DH_SN_OFFSET), &sn_info, sizeof(struct nic_sn_info));
    }
    ...
    // 打印数组的全部内容
    printk(KERN_CONT "[zxdh_pf][%s][%d] sn_code: ",  __func__, __LINE__);
    for (i = 0; i < SN_CODE_LENGTH; ++i) {
        printk(KERN_CONT "%02x ", pf_dev->sn_code[i]);
    }
    printk(KERN_CONT "\n");
...
static uint8_t zxdh_pf_fwcap_readb(struct dh_core_dev *dh_dev, uint32_t offset)
{
    ...
    return readb((const volatile void __iomem *)(uintptr_t)(vaddr + offset));
}
...
int32_t zxdh_pf_vf_qpairs_uninit(struct dh_core_dev *dh_dev)
{
    ...
    vf_qp_user_max = ioread8((void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_VF_MAX_QUEUE_USER_OFFSET));
    iowrite8(vf_qp_user_max, (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_VF_QUEUE_USER_OFFSET + pf_idx));
...
int32_t dh_pf_fw_compat_check(struct dh_core_dev *dev)
{
    ...
    fw_compat = (struct zxdh_fw_compat *)((void __iomem*)(uintptr_t)pf_dev->pci_ioremap_addr[0] + ZXDH_FW_VER_OFFSET);
    memcpy(&pf_dev->fw_compat, (uint8_t *)(uintptr_t)fw_compat, sizeof(struct zxdh_fw_compat));
    ...
    {
        pf_dev->qtlb_offset = ioread32((void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_VQ_TLB_OFFSET + 4));
        pf_dev->qtlb_offset = (pf_dev->qtlb_offset << 32) + ioread32((void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_VQ_TLB_OFFSET));
        ...
        {
            LOG_ERR("pf_dev->qtlb_offset out-off rang, (pf_dev->qtlb_offset + 2*ZXDH_MAX_QUEUES_NUM) over BAR0 size: %llx!", \
...
#define FWCAP_BAR_READ_UNIT  (4)
    ...
    {
        *((uint32_t *)&pf_dev->fwcap + idx) = *((uint32_t *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_FW_CAP_OFFSET) + idx);
    }

    pf_dev->board_type = ioread8((void __iomem*)(uintptr_t)pf_dev->pci_ioremap_addr[0] + ZXDH_FW_CAP_OFFSET + 1);
...
void zxdh_pf_vq_pairs_config(struct dh_core_dev *dh_dev)
{
    ...
    pf_qinfo = (struct zxdh_pf_queue_info *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_PF_QUEUE_INFO_OFFSET);
...
void zxdh_pf_vf_vq_pairs_config(struct dh_core_dev *dh_dev)
{
   ...
   /* vport bit[12:14] ep_id */
    ...
    addr = (uint8_t*)(uintptr_t)pf_dev->pci_ioremap_addr[0] + ZXDH_VF_QUEUE_PAIRS_OFFSET + (vf_idx / 2);
...
void zxdh_hardware_bond_files_process(struct dh_core_dev *dh_dev) {
    ...
    // 构建文件路径
    zte_snprintf_s(solid_file_path, sizeof(solid_file_path), "/etc/dinghai/net/%llx/solid", pf_dev->spec_sbdf);
    zte_snprintf_s(primary_file_path, sizeof(primary_file_path), "/etc/dinghai/net/%llx/primary", pf_dev->spec_sbdf);

    // 检查 solid、primary 文件是否都存在
    if (kern_path(solid_file_path, LOOKUP_FOLLOW, &solid_path) ||
        kern_path(primary_file_path, LOOKUP_FOLLOW, &primary_path)) {
        goto no_solid;  // 如果有任何一个文件不存在，跳转至 no_solid
    }
    ...
    // 判断 primary 文件内容
    is_primary_port = (ret == 0) ? false : true;

    // 根据 is_primary_port 的值执行相应操作
    if (is_primary_port) {
        pf_dev->is_primary_port = true;
        pf_dev->is_rdma_aux_plug = true;
    } else {
...
int zxdh_load_one(struct dh_core_dev *dh_dev)
{
    ...
    health->recovery_cnt++;
    HEAL_INFO("%s zxdh_load_one success\n", pci_name(dh_dev->pdev));
...
void zxdh_unload_one(struct dh_core_dev *dh_dev)
{
    ...
    HEAL_INFO("%s zxdh_unload_one start\n", pci_name(dh_dev->pdev));
    mutex_lock(&dh_dev->lock);
    if (dh_dev->driver_process == ZXDH_REMOVE) {
        mutex_unlock(&dh_dev->lock);
        return;
...
int32_t zxdh_user_vf_qpairs_update(struct dh_core_dev *dev, uint8_t vf_qp, uint16_t pf_idx)
{
    ...
    iowrite8(vf_qp, (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_VF_QUEUE_USER_OFFSET + pf_idx));
    return 0;
}
...
/* 更新内核态vf分配队列数信息 */
    ...
    dev_qinfo = (struct zxdh_dev_queue_info *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_DEV_QUEUE_INFO_OFFSET + pf_idx * 4);
    pf_qinfo = (struct zxdh_pf_queue_info *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_PF_QUEUE_INFO_OFFSET);
    LOG_DEBUG("pf(vport:0x%x) get queue config: total_qp:%u, start_qp_id:%u, pf_qp:%u, vf_qp:%u\n",
