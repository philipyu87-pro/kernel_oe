...
static ssize_t health_selfhealing_store(struct kobject *kobj,
			struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct zxdh_core_health *health = container_of(attr, struct zxdh_core_health, attrs[HEALTH_SELFHEALING]);
	struct zxdh_pf_device *pf_dev = container_of(health, struct zxdh_pf_device, health);
	struct dh_core_dev *dh_dev = container_of((void*)pf_dev, struct dh_core_dev, priv);
	int err = 0;
	int selfhealing;
...
static void get_m7_and_riscv_counter(struct zxdh_core_health *dh_health)
{
	...
	printk(KERN_INFO "** riscv_health_counter: 0x%x\n", riscv_count);
}
...
static void zxdh_health_info_show(struct dh_core_dev *dh_dev)
{
	...
	printk(KERN_INFO "***************** %s nic info *****************\n", pci_name(dh_dev->pdev));
	printk(KERN_INFO "** board_type: %d\n", pf_dev->board_type);
	printk(KERN_INFO "** health_version: %d\n", health->health_version);
	printk(KERN_INFO "** health_supported: %d\n", health->health_supported);
	if (!health->health_supported)
		return;
	printk(KERN_INFO "** bar_chan_valid: %d\n", pf_dev->bar_chan_valid);
	printk(KERN_INFO "** fast_unload: %d\n", pf_dev->fast_unload);
	printk(KERN_INFO "** fatal: %d\n", health->fatal);
	printk(KERN_INFO "** health->flags: %ld, 1 means ZXDH_DROP_NEW_HEALTH_WORK\n", health->flags);
	printk(KERN_INFO "** recovery_cnt: %d\n", health->recovery_cnt);
	get_m7_and_riscv_counter(health);

	printk(KERN_INFO "****************** health config ***************\n");
	printk(KERN_INFO "** DH_HEALTH_ATTR_NUM: %d\n", DH_HEALTH_ATTR_NUM);
	printk(KERN_INFO "** health->riscv.hb: 0x%p, health->m7.hb: 0x%p\n",
			health->riscv.hb, health->m7.hb);
	printk(KERN_INFO "** m7_log_offset: 0x%llx, riscv_crdump_size: 0x%llx\n",
			health->m7_log_offset, health->riscv_crdump_size);
	printk(KERN_INFO "** timer_poll: %d\n", ZXDH_HEALTH_POLL_INTERVAL);
	printk(KERN_INFO "** M7_MAX_MISSES: %d\n", M7_MAX_MISSES);
	printk(KERN_INFO "** RISCV_LOG_DUMP: %d\n", RISCV_LOG_DUMP);
	printk(KERN_INFO "** RISCV_MAX_MISSES: %d\n", RISCV_MAX_MISSES);

	printk(KERN_INFO "************ dh_dev->device_state: 0x%x **********\n", dh_dev->device_state);
	printk(KERN_INFO "** [%d]: ZXDH_DEVICE_STATE_UNINITIALIZED\n", ZXDH_DEVICE_STATE_UNINITIALIZED);
	printk(KERN_INFO "** [%d]: ZXDH_DEVICE_STATE_UP\n", ZXDH_DEVICE_STATE_UP);
	printk(KERN_INFO "** [%d]: ZXDH_DEVICE_STATE_INTERNAL_ERROR\n", ZXDH_DEVICE_STATE_INTERNAL_ERROR);

	printk(KERN_INFO "******************** synd: 0x%lx *************\n", health->synd);
	for (i = 0; i < SYND_COUNT_MAX; ++i) {
		if (i < RISCV_SYND_COUNT_MAX)
			printk(KERN_INFO "** bit[%d]: %s set %d times\n", i, synd_name[i], health->synd_statics[i]);
...
static ssize_t health_action_store(struct kobject *kobj,
			struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct zxdh_core_health *health = container_of(attr, struct zxdh_core_health, attrs[HEALTH_ACTION]);
	struct zxdh_pf_device *pf_dev = container_of(health, struct zxdh_pf_device, health);
	struct dh_core_dev *dh_dev = container_of((void*)pf_dev, struct dh_core_dev, priv);
	int err = 0;
	int action;
...
static void fw_log_dump(struct dh_core_dev *dh_dev, uint8_t type)
{
	...
	if ((pf_dev->pcie_id & BIT(14)) != 0) {
		for (i = 0; (i * EP4_DUMP_SIZE_MAX) < dump_size; ++i) {
			...
			memcpy(buf + i * EP4_DUMP_SIZE_MAX, (void __iomem*)(pf_dev->pci_ioremap_addr[0] + real_offset), EP4_DUMP_SIZE_MAX);
...
static uint8_t zxdh_health_check_fatal_sensors(struct zxdh_core_health *health)
{
	struct zxdh_pf_device *pf_dev = container_of(health, struct zxdh_pf_device, health);
	struct dh_core_dev *dh_dev = container_of((void*)pf_dev, struct dh_core_dev, priv);
	uint8_t i = 0;
...
static void zxdh_fw_fatal_err_work(struct work_struct *work)
{
	...
	HEAL_INFO("%s zxdh_fw_fatal_err_work start\n", pci_name(dh_dev->pdev));
	if (dh_dev->coredev_type == DH_COREDEV_PF)
