...
void zxdh_en_sf_set_queue_size(struct dh_core_dev *dh_dev, uint32_t index, uint16_t size)
{
    struct zxdh_en_sf_device *en_sf_dev = dh_core_priv(dh_dev);
...
struct zxdh_rdma_dev_info *zxdh_rdma_infos_init(struct dh_core_dev *dh_dev, struct zxdh_auxiliary_device *adev)
{
    ...
    rdma_infos->ver.support = ZXDH_NET_MAJOR_VER + (ZXDH_NET_MINOR_VER << ZXDH_HIGH_8BIT) + (ZXDH_RDMA_MINOR_VER << ZXDH_HIGH_16BIT);
...
static int32_t zxdh_en_sf_dev_probe(struct zxdh_auxiliary_device *adev, const struct zxdh_auxiliary_device_id *id)
{
    ...
    if (sf_ops->en_sf_is_rdma_enable(dh_dev->parent) && sf_ops->en_sf_is_primary_port(dh_dev->parent, TRUE, FALSE))
    {
