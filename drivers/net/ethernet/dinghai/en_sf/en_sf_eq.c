...
uint16_t zxdh_en_sf_get_queue_vector(struct dh_core_dev *dh_dev, uint16_t channel, uint16_t queue_index, uint16_t vq_idx)
{
    struct zxdh_en_sf_device *en_sf_dev = dh_core_priv(dh_dev);
    struct dh_eq_table *table = &dh_dev->eq_table;
    struct dh_en_sf_eq_table *sf_eq_table = table->priv;
    ...
    msix_vec = en_sf_dev->sf_ops->en_sf_get_queue_vector(dh_dev->parent, channel, &sf_eq_table->vqs_eqs_list, queue_index, vq_idx);
...
int32_t zxdh_en_sf_vq_bind_channel(struct dh_core_dev *dh_dev, int32_t channel_num, int32_t queue_index, uint16_t vq_idx)
{
    int32_t msix_vec = ZXDH_MSI_NO_VECTOR;
