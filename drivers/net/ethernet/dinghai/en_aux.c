...
#define KERNEL_PF_FC   92500 /* 支持VQM PF默认92.5G(94.5)限速 */
#define KERNEL_PF_FC_1 23750 /* 支持VQM PF默认23.75G(23.75)限速 */
...
static int32_t en_open(struct net_device *netdev, bool boot)
{
    ...
    for (i = 0; i < en_dev->max_vq_pairs; i++)
    {
        ...
        err = xdp_rxq_info_reg(&en_dev->rq[i].xdp_rxq, netdev, i, en_dev->rq[i].napi.napi_id);
        if (err < 0)
        {
        ...
        err = xdp_rxq_info_reg_mem_model(&en_dev->rq[i].xdp_rxq, MEM_TYPE_PAGE_SHARED, NULL);
        if (err < 0)
        {
            mutex_unlock(&en_priv->lock);
...
void pkt_protocol_parse(struct sk_buff *skb, struct zxdh_net_hdr_tx *hdr, int32_t flag)
{
    ...
    else
    {
        ...
        else if ((mach->h_proto == htons(ETH_P_IPV6)) || (l3_protocol == htons(ETH_P_IPV6)))
        {
...
static int pd_hdr_validate_vlan(struct zxdh_en_device *en_dev, struct sk_buff *skb, struct zxdh_net_hdr_tx *hdr)
{
    if (skb && skb_vlan_tag_present(skb))
    {
...
void zxdh_netdev_features_over_dtp(struct net_device *netdev)
{
    ...
    LOG_DEBUG("dtp_drs_offload=%d. TSO:%lld, TSO6:%lld, HWCSUM:%lld, GSO_UDP_TUNNEL_CSUM:%lld\n", en_dev->dtp_drs_offload,
              netdev->features & NETIF_F_TSO, netdev->features & NETIF_F_TSO6, netdev->features & NETIF_F_HW_CSUM,
              netdev->features & NETIF_F_GSO_UDP_TUNNEL_CSUM);
    ...
    if (en_dev->enable_1588 == true)
    {
        if (is_1588_flag == IS_1588_MESSAGE)
        {
            if(en_dev->dtp_drs_offload == true)
            {
                ...
                if (ret != 0)
                {
                    DEBUG_1588("dev %s vport 0x%x pkt_1588_proc_xmit ERR, ret: %d\n", en_dev->netdev->name, en_dev->vport,ret);
                }
                DEBUG_1588("NET HDR:");
            ...
            else
            {
                ...
                if (ret != 0)
                {
                    LOG_ERR("dev %s vport 0x%x pkt_1588_proc_nopi_xmit ERR, ret: %d\n", en_dev->netdev->name, en_dev->vport, ret);
                    return ret;
                }
                DEBUG_1588("NET HDR:");
...
{
    if (ether_addr_equal(BOND_MCAST_ADDR, addr) || is_standard_predefined_ipv6_multicast_mac(addr))
    {
        return true;
    }
    return false;
...
int32_t zxdh_dev_promisc_sync(struct zxdh_en_device *en_dev)
{
    ...
    if ((en_dev->promisc_enabled == promisc_changed) && (en_dev->allmulti_enabled == allmulti_changed))
        return 0;
                            ...
                            struct netlink_ext_ack *extack)
{
    ...
    if ((dev->features & NETIF_F_GRO_HW) || (dev->features & NETIF_F_HW_CSUM)) {
        LOG_ERR("Can't set XDP while host is implementing GRO_HW/CSUM, disable GRO_HW/CSUM first\n");
        return -EOPNOTSUPP;
    }

    if (en_dev->mergeable_rx_bufs && !en_dev->any_header_sg) {
        LOG_ERR("XDP expects header/data in single page, any_header_sg required\n");
        return -EINVAL;
    }
...
bool ipv6_mac_refcount_get(struct zxdh_en_device *en_dev, const uint8_t *ip6mac, int32_t *ipv6_mac_refconut)
{
    ...
    mutex_unlock(&ip6mac_tbl->mlock);
    LOG_INFO("ipv6_mac_refcount_get end\n");
    return false;
}
                          ...
                          const uint8_t *addr, uint16_t flags)
{
    ...
    LOG_INFO("multicast_mac_add end\n");
    return 0;
}
...
int32_t zxdh_ip6mac_add(struct zxdh_en_device *en_dev, const uint32_t *addr6, const uint8_t *ip6mac)
{
    ...
    list_for_each_entry(cte, &ip6mac_tbl->hash_list[mac_hash_val], list)
    {
        ...
        {//MAC已经存在
            ...
            DH_LOG_DEBUG(MODULE_PF, "Eth:%s, Increase Multicast MAC Address(%pM) refcnt:%d\n", en_dev->netdev->name, ip6mac, refcount_read(&ce->refcnt));
            mutex_unlock(&ip6mac_tbl->mlock);
            return 0;
        }
    }
...
found:
    ...
    en_dev->curr_multicast_num--;
    DH_LOG_DEBUG(MODULE_PF, "curr_multicast_num is %d\n", en_dev->curr_multicast_num);
    DH_LOG_DEBUG(MODULE_PF, "Eth:%s, Del Multicast MAC Address: %pM Completely, refcnt:%d, np ret:%d\n", en_dev->netdev->name, ip6mac, refcnt, err);
    return err;
}
...
int32_t zxdh_pf_add_vf_unicast_mac(struct zxdh_en_device *en_dev, struct dhtool_set_vf_mac_msg *msg)
{
    ...
    LOG_INFO("zxdh_pf_add_vf_unicast_mac is called\n");
...
int32_t zxdh_pf_add_vf_multicast_mac(struct zxdh_en_device *en_dev, struct dhtool_set_vf_mac_msg *msg)
{
    ...
    LOG_INFO("zxdh_pf_add_vf_multicast_mac is called\n");
...
int32_t zxdh_pf_del_vf_multicast_mac(struct zxdh_en_device *en_dev, struct dhtool_set_vf_mac_msg *msg)
{
    ...
    LOG_INFO("zxdh_pf_del_vf_multicast_mac is called\n");
...
int32_t zxdh_pf_transfer_vf_mac(struct zxdh_en_device *en_dev, uint32_t src_vf, uint32_t dst_vf)
{
    ...
    LOG_INFO("zxdh_pf_transfer_vf_mac is called\n");
    ...
    LOG_INFO("zxdh_pf_transfer_vf_mac success\n");
    return MAC_CONFIG_SUCCESS;
}
...
static int32_t zxdh_pf_set_vf_port_vlan(struct zxdh_en_device *en_dev, int vf_idx, u16 vid, u8 qos, uint16_t vlan_proto)
{
    ...
    struct zxdh_vf_item *vf_item = NULL;
    DPP_PF_INFO_T pf_info = {0};
    uint16_t new_vlan_tci = ZXDH_VLAN_TCI_GEN(vid, qos);
    ...
    for (i=0; i<current_unicast_num; i++)
    {
        if (vf_item->vport == unicast_mac_arry[i].vport)
        {
            retval = dpp_unicast_mac_search(&pf_info, unicast_mac_arry[i].addr, htons(vlan_proto), new_vlan_tci, &current_vport);
            if ((retval == 0) && (vf_item->vport != current_vport))
            {
                LOG_ERR("modify vlan failed, [Mac]+[vlan] Already exists: current_vport=0x%04x\n", current_vport);
                LOG_ERR("new_vlan_tci=%d; htons(vlan_proto)=%d\n", new_vlan_tci, htons(vlan_proto));
    ...
    {
        retval = dpp_unicast_mac_search(&pf_info, vf_item->mac, htons(vlan_proto), new_vlan_tci, &current_vport);
        if ((retval == 0) && (current_vport != vf_item->vport)) {
            LOG_ERR("modify vlan failed, [Itm Mac]+[vlan] Already exists np: current_vport=0x%04x\n", current_vport);
            LOG_ERR("new_vlan_tci=%d; htons(vlan_proto)=%d\n", new_vlan_tci, htons(vlan_proto));
        ...
        for (i = 0; i < num_vfs; i++) {
            ...
            if (ether_addr_equal(cur_vf_item->mac, vf_item->mac) && \
                ...
                LOG_ERR("modify vlan failed, [Itm Mac]+[vlan] Already exists: current_vport=0x%04x\n", current_vport);
                LOG_ERR("new_vlan_tci=%d; htons(vlan_proto)=%d\n", new_vlan_tci, htons(vlan_proto));
                LOG_ERR("mac = %x %x %x %x %x %x\n", vf_item->mac[0],vf_item->mac[1],vf_item->mac[2],\
                                                    vf_item->mac[3],vf_item->mac[4],vf_item->mac[5]);
                retval = -EEXIST;
    ...
    if (vid)
    {
        ...
        {
            retval = zxdh_handle_mac_operations(&pf_info, unicast_mac_arry, current_unicast_num, vf_item, old_vlan_tci, vlan_proto, 0);
            if (retval != 0)
            {
            ...
            retval = zxdh_handle_mac_operations(&pf_info, unicast_mac_arry, current_unicast_num, vf_item, new_vlan_tci, vlan_proto, 1);
            if (retval != 0)
            {
...
int zxdh_en_ndo_set_vf_mac(struct net_device *netdev, int vf_id, u8 *mac)
{
    ...
    for (i = 0; i < num_vfs; i++)
    {
        ...
        if (ether_addr_equal(cur_vf_item->mac, mac) && \
           ((ZXDH_VLAN_TCI_GEN(cur_vf_item->vlan, cur_vf_item->qos) == sriov_vlan_id) && (cur_vf_item->vlan_proto == vf_item->vlan_proto)))
        {
...
int zxdh_en_ndo_set_tx_maxrate(struct net_device *netdev, int qid, uint32_t max_rate)
{
    ...
    //条件1，4G以内满足配置能满足vqm限速周期全局共享,ECO回片可放宽限速值
    ...
    if (max_rate < 4000)
    {
        ...
        PLCR_LOG_INFO("The Rate of VF item:%d has been set to: Max Tx Rate: %dMbit/s in vqm\n",
                        qid, max_rate);
    }
    return rtn;
}
...
int zxdh_en_ndo_set_vf_rate(struct net_device *netdev, int vf_id, int min_tx_rate, int max_tx_rate)
{
    ...
    if (!en_dev->ops->get_vf_is_probe(en_dev->parent, vf_id))
    {
        LOG_INFO("zxdh_en_ndo_set_vf_rate, vf %d is not probed\n", vf_id);
        return 0;
    }
...
int32_t ptp_set_pf_uplink_vfid(struct zxdh_en_device *en_dev)
{
    ...
    LOG_INFO("vport: 0x%x, IS_PF: %d VFID %u", en_dev->vport, IS_PF(en_dev->vport), VQM_VFID(en_dev->vport));
...
void zxdh_cfg_vqm_vf_fc_kbps(struct zxdh_en_device *en_dev, uint32_t pf_fc_val)
{
    ...
    else
    {
        ...
        {
            LOG_ERR("cfg vqm vf(%u) rate %ukbps failed!\n", vqm_vfid, pf_fc_val);
        }
        else
        {
            LOG_INFO("cfg vqm vf(%u) rate %ukbps success.\n", vqm_vfid, pf_fc_val);
        }
    }
    kfree(msg);
}
...
static int32_t zxdh_en_dev_probe(struct zxdh_auxiliary_device *adev, const struct zxdh_auxiliary_device_id *id)
{
    ...
    LOG_INFO("ep_bdf: 0x%x, vport: 0x%x, pcie_id: %d, slot_id: %d. is_bond %d\n", en_dev->ep_bdf, \
             en_dev->vport, en_dev->pcie_id, en_dev->slot_id, en_dev->ops->is_bond(en_dev->parent));
    if (!en_dev->ops->is_bond(en_dev->parent))
    {
        ...
        ZXDH_SET_PFLAG(en_dev->pflags, ZXDH_PFLAG_HARDWARE_BOND_PRIMARY, en_dev->is_primary_port ? 1 : 0);
    }
#ifdef ZXDH_MSGQ
    IS_MSGQ_DEV(en_dev)
    {
    ...
    if(en_dev->ops->get_coredev_type(en_dev->parent) == DH_COREDEV_VF)
    {
        ...
        {
            LOG_INFO("VF device: %s vqm_link_state %d not equal to kernel_link_state %d\n", pci_name(en_dev->ops->get_pdev(en_dev->parent)), link_up, carrier_status);
