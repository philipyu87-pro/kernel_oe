...
int32_t zxdh_spinlock_lock(uint32_t virt_lock_id, uint64_t lock_virt_addr, uint64_t label_addr, uint16_t masterid)
{
    ...
    {
        /* 读寄存器加锁 */
        spl_val = spinklock_read((spl_addr_t)(uintptr_t)lock_virt_addr, virt_lock_id);
        if (0 == spl_val)
        {
            label_write((spl_addr_t)(uintptr_t)label_addr, virt_lock_id, masterid);
            break;
        }
        lock_rd_cnt++;
    ...
    {
        BAR_LOG_ERR("spl val: 0x%x lock_id:%u, masterID: 0x%x.\n", spl_val, virt_lock_id, label_read((spl_addr_t)(uintptr_t)label_addr, virt_lock_id));
...
{
    label_write((spl_addr_t)(uintptr_t)label_addr, virt_lock_id, 0);
    spinlock_write((spl_addr_t)(uintptr_t)lock_virt_addr, virt_lock_id, 0);
    return 0;
}
