    ...
    DPP_PF_INFO_T *pf_info, struct zxdh_vf_item *vf_item, struct zxdh_pf_device *pf_dev)
{
    ...
    uint16_t sriov_vlan_id = ZXDH_VLAN_TCI_GEN(vf_item->vlan, vf_item->qos);

    LOG_INFO("zxdh_vf_port_init, vfindex%d\n", vf_idx);
    ...
    LOG_INFO("zxdh_vf_port_init plcr %u\n",vf_item->max_tx_rate);
    ...
    DPP_PF_INFO_T *pf_info, struct zxdh_vf_item *vf_item, struct zxdh_pf_device *pf_dev)
{
    ...
    LOG_INFO("zxdh_vf_port_reload, vfindex%d\n", vf_idx);
    ret = dpp_vport_create(pf_info);
    ...
    DPP_PF_INFO_T *pf_info, struct zxdh_vf_item *vf_item, struct zxdh_pf_device *pf_dev)
{
    ...
    {
        LOG_ERR("vf multicast mac num:%u beyond 32", vf_item->vf_mac_info.current_multicast_num);
                                 ...
                                 const unsigned char *target_mac)
{
    ...
    for (i = 0; i < num_vfs; i++)
    {
        ...
        if (ether_addr_equal(cur_vf_item->mac, target_mac) && \
           ((ZXDH_VLAN_TCI_GEN(cur_vf_item->vlan, cur_vf_item->qos) == sriov_vlan_id) &&  (cur_vf_item->vlan_proto == sriov_vlan_tpid)))
        {
    ...
    DPP_PF_INFO_T *pf_info, struct zxdh_vf_item *vf_item, struct zxdh_pf_device *pf_dev)
{
    ...
    {
        LOG_ERR("vf unicast mac num:%u beyond 128", vf_item->vf_mac_info.current_unicast_num);
        mutex_unlock(&vf_item->lock);
        return ZXDH_REPS_BEYOND_MAC;
    }
    ...
    DPP_PF_INFO_T *pf_info, struct zxdh_vf_item *vf_item, struct zxdh_pf_device *pf_dev)
{
    ...
    {
        err = dpp_vport_attr_set(pf_info, SRIOV_VPORT_IP_CHKSUM, msg->port_attr_set_msg.value);
        if (err != 0)
        {
    ...
    DPP_PF_INFO_T *pf_info, struct zxdh_vf_item *vf_item, struct zxdh_pf_device *pf_dev)
{
    ...
    LOG_DEBUG("zxdh_vf_np_stats_get is called, vport: 0x%x, vf_id %d, pf_info.slot %u\n", pf_info->vport, vf_id, pf_info->slot);
