...
static void link_info_irq_update_vf_bond_pf_work_handler(struct work_struct *_work)
{
    ...
    for (vf_idx = 0; vf_idx < num_vfs; vf_idx++)
    {
        ...
        if(vf_item->link_forced == FALSE && vf_item->is_probed)
        {
            ...
            LOG_DEBUG("[pf_level] bond_pf pcie_id:0x%x write phyport[0x%x] and link_up[%d] to VF[%d] VQM[0x%x]\n", pf_dev->pcie_id, phyport_val, link_up_val, vf_idx, link_info);
        }
    }
...
static void mac_info_pf_work_handler(struct work_struct *_work)
{
    ...
    LOG_INFO("mac_info_pf_work_handler is call\n");
    ZXDH_AUX_COMP_FLAG_CHECK(pf_dev);
    ...
    for (i=0; i<num_vfs; i++)
    {
        val = ioread8((void __iomem *)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_MAC_FLAG_BAR_OFFSET + ep_id * ZXDH_EP_FLAG_SIZE + pf_id * ZXDH_PF_FLAG_SIZE + i));
        if (val == 1)
        {
            vf_flag[sum_flag] = i;
            iowrite8(0, (void __iomem*)(uintptr_t)(pf_dev->pci_ioremap_addr[0] + ZXDH_MAC_FLAG_BAR_OFFSET + ep_id * ZXDH_EP_FLAG_SIZE + pf_id * ZXDH_PF_FLAG_SIZE + i));
            sum_flag ++;
        }
    }
    ...
    for (i=0; i<sum_flag; i++)
    {
        ...
        for (j=0; j< current_unicast_num; j++)
        {
            if (vf_vport == unicast_mac_arry[j].vport)
            {
                ...
                LOG_INFO("dpp_del_mac sucess, vport: 0x%x\n", vf_vport);
            }
        }
