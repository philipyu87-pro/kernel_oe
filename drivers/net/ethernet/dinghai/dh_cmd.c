...
uint16_t bar_msg_src_parse(struct zxdh_pci_bar_msg *in)
{
    ...
    if (in->src == MSG_CHAN_END_MPF)
    {
        ...
        in->virt_addr = (uint64_t)(uintptr_t)internal_addr + BAR_MSG_OFFSET;
...
int bar_chan_reg_write(uint64_t subchan_addr, uint32_t offset, uint32_t data)
{
    ...
    writel(data, (volatile void*)(uintptr_t)(subchan_addr + algin_offset));
    return 0;
}
...
int bar_chan_reg_read(uint64_t subchan_addr, uint32_t offset, uint32_t *pdata)
{
    ...
    *pdata = readl((const volatile void *)(uintptr_t)(subchan_addr + algin_offset));
    return 0;
}
...
int bar_chan_sync_msg_reps_get(uint64_t subchan_addr, uint64_t recv_buffer, uint16_t buffer_len, uint16_t send_msg_id, uint16_t op_code)
{
    int ret = BAR_MSG_OK;
    uint16_t recv_msg_id = 0;
    uint16_t recv_len = 0;
    uint8_t *recv_msg =  (uint8_t*)(uintptr_t)recv_buffer;
    ...
    if ((op_code < ZXDH_GET_SW_STATS) && (recv_len > ZXDH_REPS_MAX_SIZE_BEFORE57)) //op_code=57之前的PF VF兼容性场景考虑
    {
...
int zxdh_bar_chan_sync_msg_send(struct zxdh_pci_bar_msg *in, struct zxdh_msg_recviver_mem *result)
{
    ...
    if (*(uint32_t*)(uintptr_t)subchan_addr == 0xffffffff)
    {
    ...
    else
    {
        ...
        ret = bar_chan_sync_msg_reps_get(subchan_addr, (uint64_t)(uintptr_t)result->recv_buffer, result->buffer_len, msg_id, op_code);
    }
free_chan:
    bar_chan_msgid_free(msg_id);
    /*通道解锁*/
