...
static int32_t zxdh_hardware_bond_enable_proc(struct net_device *netdev, bool enable)
{
    ...
    if (netif_is_lag_port(netdev)) //当前正处于 linux bond 场景，不允许配置rdma硬bond功能
    {
    ...
    return en_dev->ops->update_hb_file_val(en_dev->parent, en_dev->spec_sbdf, "solid", en_dev->is_hwbond);
}
...
static int32_t zxdh_hardware_bond_primary_enable_proc(struct net_device *netdev, bool enable)
{
    ...
    if (en_dev->ops->is_bond(en_dev->parent) || (en_dev->ops->is_special_bond(en_dev->parent)) || (en_dev->ops->get_coredev_type(en_dev->parent) == DH_COREDEV_VF) || (!zxdh_en_is_panel_port(en_dev)))
        return 0;

    if (netif_is_lag_port(netdev)) //当前正处于 linux bond 场景，不允许进行主次口配置。
    {
    ...
    // 检查 primary 文件是否存在，并向其内部写值
    return en_dev->ops->update_hb_file_val(en_dev->parent, en_dev->spec_sbdf, "primary", en_dev->is_primary_port);
}
...
static int32_t zxdh_ets_info_update(struct zxdh_en_priv *en_priv, uint32_t mode, uint32_t *cur_mode, uint32_t *tc_td_th)
{
    ...
    *cur_mode = en_dev->ets_info.cur_ets; // 传出当前开关状态
    ...
    else if(en_dev->ets_info.cur_ets) // 只有当前开关状态是开的状态下才更新tc_td_th
    {
        ...
        LOG_INFO("td_th -> tc_td_th[0]:%d tc_td_th[1]:%d tc_td_th[2]:%d tc_td_th[3]:%d tc_td_th[4]:%d tc_td_th[5]:%d tc_td_th[6]:%d tc_td_th[7]:%d\n",
...
static int32_t zxdh_ets_switch_proc(struct net_device *netdev, bool enable)
{
    ...
    if((old_mode ^ mode) && mode) // ets开关状态发生改变，ets 关->开
    {
        ...
        LOG_INFO("set_flow_td_th -> tc_td_th[0]:%d tc_td_th[1]:%d tc_td_th[2]:%d tc_td_th[3]:%d tc_td_th[4]:%d tc_td_th[5]:%d tc_td_th[6]:%d tc_td_th[7]:%d\n",
                                                    tc_td_th[0], tc_td_th[1], tc_td_th[2], tc_td_th[3], tc_td_th[4], tc_td_th[5], tc_td_th[6], tc_td_th[7]);
        if(ret)
        {
...
static void zxdh_en_get_strings(struct net_device *netdev, u32 stringset, u8 *data)
{
    ...
    switch (stringset)
    {
        ...
        case ETH_SS_PRIV_FLAGS:
        {
            ...
            if ((pf_dev->board_type == DH_STDA) || (pf_dev->board_type == DH_STDB)|| (pf_dev->board_type == DH_STDC)
                || (pf_dev->board_type == DH_STD_E312S))
            {
...
int32_t zxdh_stats_update(struct zxdh_en_device *en_dev)
{
    ...
    for (i = 0; i < en_dev->curr_queue_pairs; i++)
    {
        ...
        en_dev->hw_stats.netdev_stats.rx_csum_offload_good += en_dev->rq[i].stats.rx_csum_offload_good;
...
static void zxdh_en_get_ethtool_stats(struct net_device *netdev, struct ethtool_stats *stats, u64 *data)
{
    struct zxdh_en_priv *en_priv = netdev_priv(netdev);
    struct zxdh_en_device *en_dev = &en_priv->edev;
    uint32_t offset = ZXDH_NETDEV_STATS_NUM + ZXDH_MAC_STATS_NUM + ZXDH_VPORT_STATS_NUM + ZXDH_UDP_STATS_NUM;
...
{
    /* Started by AICoder, pid:3de164760c033b9146ed0b10407e2720681505ce */
...
static int zxdh_en_set_channels(struct net_device *netdev, struct ethtool_channels *ch)
{
    ...
    LOG_DEBUG("zxdh_en_set_channels start\n");
    ZXDH_DEVICE_STATE_CHECK_RTN(en_dev);
...
static int32_t zxdh_ethtool_get_flow(struct zxdh_en_device *en_dev, struct ethtool_rxnfc *info, int32_t location)
{
    ...
    if (en_dev->ops->get_coredev_type(en_dev->parent) == DH_COREDEV_PF) {
        err = dpp_tbl_fd_cfg_get(&pf_info, ZXDH_SDT_FD_CFG_TABLE, en_dev->fs.ethtool_fs[location].index, &p_fd_cfg);
        if (err != 0) {
...
static int32_t zxdh_ethtool_get_all_flows(struct zxdh_en_device *en_dev, struct ethtool_rxnfc *info, uint32_t *rule_locs)
{
    ...
    LOG_INFO("zxdh_ethtool_get_all_flows rule_cnt:%d\n", info->rule_cnt);
    ...
    {
        LOG_INFO("zxdh_ethtool_get_all_flows idx:%d less than %d\n", idx, info->rule_cnt);
    }
    return err;
}
                                ...
                                uint32_t index)
{
    ...
    LOG_INFO("set_flow_table: location is %u, index is %u\n", fs->location, index);
    return ;
}
...
static int32_t zxdh_ethtool_flow_replace(struct zxdh_en_device *en_dev, struct ethtool_rx_flow_spec *fs)
{
    ...
    LOG_INFO("zxdh_ethtool_flow_replace start!\n");
...
static int32_t zxdh_ethtool_flow_remove(struct zxdh_en_device *en_dev, int32_t location)
{
    ...
    LOG_INFO("zxdh_ethtool_flow_remove start!\n");
                ...
                struct kernel_ethtool_coalesce *kec, struct netlink_ext_ack *ack)
{
    struct zxdh_en_priv *en_priv = netdev_priv(netdev);
    struct zxdh_en_device *en_dev = &en_priv->edev;
    uint16_t rx_msix_mode = 0;
    uint16_t tx_msix_mode = 0;
    uint32_t rx_coalesce_usecs = 0;
    uint32_t tx_coalesce_usecs = 0;
    int32_t err = 0;

    ZXDH_DEVICE_STATE_CHECK_RTN(en_dev);
    LOG_DEBUG("zxdh_en_get_coalesce start\n");
    ...
    LOG_DEBUG("zxdh_en_get_coalesce end\n");
    return 0;
}
                ...
                struct kernel_ethtool_coalesce *kec, struct netlink_ext_ack *ack)
{
    ...
    ZXDH_DEVICE_STATE_CHECK_RTN(en_dev);
    LOG_DEBUG("zxdh_en_set_coalesce start\n");
    if (coal->tx_coalesce_usecs > ZXDH_MAX_COAL_TIME || coal->rx_coalesce_usecs > ZXDH_MAX_COAL_TIME)
    {
