...
struct zxdh_en_netdev_stats
{
    ...
    uint64_t rx_csum_offload_error;
};
...
#define DEV_UNICAST_MAX_NUM     128    /* 每个PF/VF存储的单播mac转发表上限 */
...
struct zxdh_en_if {
    ...
    int32_t (*vq_bind_channel)(struct dh_core_dev *dh_dev, int32_t channel_num, int32_t queue_index, uint16_t vq_idx);
    ...
    void (*set_queue_size)(struct dh_core_dev *dh_dev, uint32_t index, uint16_t size);
