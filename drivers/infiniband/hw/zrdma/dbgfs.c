...
int write_np_cnp_prio_mode(struct zxdh_pci_f *rf, u32 var)
{
	...
	SET_32_REG_VAL(rf, RDMA_DCQCN_NP_CNP_PRIO_MODE,
...
int write_np_min_time_between_cnps(struct zxdh_pci_f *rf, u32 var)
{
	...
	y = FIELD_GET(ZXDH_DCQCN_NP_MIN_TIME_BETWEEN_CNPS_Y,
		      rd32(rf->sc_dev.hw,
			   RDMA_DCQCN_NP_MIN_TIME_BETWEEN_CNPS_Y));
	y_ex = FIELD_GET(ZXDH_DCQCN_NP_MIN_TIME_BETWEEN_CNPS_Y_EX,
			 rd32(rf->sc_dev.hw,
			      RDMA_DCQCN_NP_MIN_TIME_BETWEEN_CNPS_Y_EX));
...
int write_rpg_byte_reset(struct zxdh_pci_f *rf, u32 var)
{
	...
	SET_32_REG_VAL(rf, RDMA_DCQCN_RPG_BYTE_RESET, ZXDH_DCQCN_RPG_BYTE_RESET,
		       var);
	return 0;
}
...
int write_rpg_hai_rate(struct zxdh_pci_f *rf, u32 var)
{
	...
	SET_32_REG_VAL(rf, RDMA_DCQCN_RPG_HAI_RATE, ZXDH_DCQCN_RPG_HAI_RATE,
		       var);
	return 0;
}
...
int read_psn_wraparound_enable(struct zxdh_pci_f *rf, u32 *var)
{
	...
	if (!cdev_info) {
		*var = 0;
		pr_err("read_psn_wraparound_enable: cdev_info is null!\n");
		return -EIO;
	}
	...
	if (np_ret) {
		*var = 0;
		pr_err("read_psn_wraparound_enable: query dpp_glb_cfg_get_1 failed!\n");
		return -EIO;
	}
...
int write_psn_wraparound_enable(struct zxdh_pci_f *rf, u32 var)
{
	...
	cdev_info = rf->cdev;
	if (!cdev_info) {
		pr_err("write_psn_wraparound_enable: cdev_info is null!\n");
		return -EIO;
	}
	pf_info.vport = cdev_info->vport_id;
	pf_info.slot = cdev_info->slot_id;
	...
	if (np_ret) {
		pr_err("write_psn_wraparound_enable: query dpp_pktrx_mcode_glb_cfg_write failed!\n");
		return -EIO;
	}
	return 0;
}
...
int read_pmtu(struct zxdh_pci_f *rf, u32 *var)
{
	...
	if (!cdev_info) {
		*var = 0;
		pr_err("read_pmtu: cdev_info is null!\n");
		return -EIO;
	}
	pf_info.vport = cdev_info->vport_id;
	pf_info.slot = cdev_info->slot_id;
	...
	if (np_ret) {
		*var = 0;
		pr_err("read_pmtu: query dpp_glb_cfg_get_1 failed!\n");
		return -EIO;
	}
...
int write_pmtu(struct zxdh_pci_f *rf, u32 var)
{
	...
	cdev_info = rf->cdev;
	if (!cdev_info) {
		pr_err("write_pmtu: cdev_info is null!\n");
		return -EIO;
	}
	pf_info.vport = cdev_info->vport_id;
	pf_info.slot = cdev_info->slot_id;
	...
	np_ret = dpp_pktrx_mcode_glb_cfg_write(&pf_info, ZXDH_NP_PMTU_START_BIT,
					       ...
					       dpp_glb_cfg_pmtu);
	if (np_ret) {
		pr_err("write_pmtu: query dpp_pktrx_mcode_glb_cfg_write failed!\n");
		return -EIO;
	}
	return 0;
}
