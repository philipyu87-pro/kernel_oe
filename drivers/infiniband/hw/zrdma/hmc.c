...
int zxdh_sc_create_date_cap_obj(struct zxdh_sc_dev *dev)
{
	...
	pr_err("HMC: zxdh_sc_create_date_cap_obj sd_lmt:%d\n", sd_lmt);
				       ...
				       struct zxdh_vfdev *vf_dev, u32 rsrc_type)
{
	...
	if (!sd_entry->valid) {
		...
		if (!dma_mem.va)
		{
			pr_info("%s %d failed to alloc mem\n", __func__, __LINE__);
			return -ENOMEM;
		}
		...
		smmu_pte_cfg.size = alloc_len;
		smmu_pte_cfg.stream_id = dev->hmc_fn_id; // 这个SID后续需要修改
		smmu_pte_cfg.access_perm = 0x03;
		// 这里调用SMMU接口
		ret = zxdh_smmu_set_pte(&smmu_pte_cfg, dev);
		if (ret)
			  ...
			  struct zxdh_hmc_pble_rsrc *pble_rsrc, u32 pages)
{
	...
	if (!sd_entry->valid) {
		...
		if (false == pble_rsrc->dev->hmc_use_dpu_ddr) { // is HOST DDR
			...
			dma_mem.size =
				ALIGN(alloc_len, ZXDH_HMC_PD_BP_BUF_ALIGNMENT);
			dma_mem.va =
				dma_alloc_coherent(hw->device, dma_mem.size,
						   &dma_mem.pa, GFP_KERNEL);
			if (!dma_mem.va)
				return -ENOMEM;
			memset(dma_mem.va, 0, dma_mem.size);
				    ...
				    u16 iw_vf_idx, u16 vf_id)
{
	...
	for (i = 0; i < ZXDH_HMC_IW_MAX; i++) {
		if ((i == ZXDH_HMC_IW_IRD) || (i == ZXDH_HMC_IW_TXWINDOW)) {
			...
			hmc_obj[i].max_cnt =
				dev->hmc_pf_manager_info.vf_pblequeue_cnt;
			hmc_obj[i].cnt =
				dev->hmc_pf_manager_info.vf_pblequeue_cnt;
			hmc_obj[i].size = dev->hmc_info->hmc_obj[i].size;
			hmc_obj[i].type = dev->hmc_info->hmc_obj[i].type;
...
struct zxdh_vfdev *zxdh_pf_get_vf_hmc_res(struct zxdh_sc_dev *dev, u16 vf_id)
{
	...
	if (!virt_mem.va) {
		pr_err("VIRT: VF%u Unable to allocate a VF device structure.\n",
		       vf_id);
		spin_unlock_irqrestore(&dev->vf_dev_lock, flags);
		return NULL;
	}
			       ...
			       u32 rsrc_type, u16 vhca_id)
{
	...
	case ZXDH_HMC_IW_PBLE_MR:
		...
		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_PBLEMR_RDMAIO_BASE_LOW));
		writel(base_high,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr +
				       C_HMC_PBLEMR_RDMAIO_BASE_HIGH));
		break;
	case ZXDH_HMC_IW_PBLE:
		...
		writel(base_low,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr +
				       C_HMC_PBLEQUEUE_RDMAIO_BASE_LOW));
		writel(base_high,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr +
				       C_HMC_PBLEQUEUE_RDMAIO_BASE_HIGH));
		break;
	case ZXDH_HMC_IW_MR:
		val = zxdh_hmc_register_config_comval(dev, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_MRTE_TX1));
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_MRTE_TX3));

		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_MRTE_RX1));
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_MRTE_RX2));

		val = zxdh_hmc_register_config_cqpval(
			dev, obj_info[rsrc_type].max_cnt, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_MRTE_CQP));

		if (dev->hmc_use_dpu_ddr == true) {
			writel(ZXDH_INDICATE_DPU_DDR,
			       (u32 __iomem *)(cqp->dev->hw->hw_addr +
					       C_HMC_MRTE_RDMAIO_INDICATE));
		} else {
			writel(ZXDH_INDICATE_HOST_SMMU,
			       (u32 __iomem *)(cqp->dev->hw->hw_addr +
					       C_HMC_MRTE_RDMAIO_INDICATE));
		...
		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_MRTE_RDMAIO_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_MRTE_RDMAIO_BASE_HIGH));
		break;
	case ZXDH_HMC_IW_AH:

		val = zxdh_hmc_register_config_comval(dev, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_AH_TX));

		val = zxdh_hmc_register_config_cqpval(
			dev, obj_info[rsrc_type].max_cnt, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_AH_CQP));

		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_AH_RDMAIO_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_AH_RDMAIO_BASE_HIGH));
		if (dev->hmc_use_dpu_ddr == true) {
			writel(ZXDH_INDICATE_DPU_DDR,
			       (u32 __iomem *)(cqp->dev->hw->hw_addr +
					       C_HMC_AH_RDMAIO_INDICATE));
		} else {
			writel(ZXDH_INDICATE_HOST_SMMU,
			       (u32 __iomem *)(cqp->dev->hw->hw_addr +
					       C_HMC_AH_RDMAIO_INDICATE));
	...
	case ZXDH_HMC_IW_IRD:
		...
		val = zxdh_hmc_register_config_comval(dev, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_IRD_RX1));
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_IRD_RX2));
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_IRD_RX3));

		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_IRD_RDMAIO_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_IRD_RDMAIO_BASE_HIGH));
		if (dev->hmc_use_dpu_ddr == true) {
			writel(ZXDH_INDICATE_DPU_DDR,
			       (u32 __iomem *)(cqp->dev->hw->hw_addr +
					       C_HMC_IRD_RDMAIO_INDICATE));
		} else {
			writel(ZXDH_INDICATE_HOST_SMMU,
			       (u32 __iomem *)(cqp->dev->hw->hw_addr +
					       C_HMC_IRD_RDMAIO_INDICATE));
	...
	case ZXDH_HMC_IW_TXWINDOW:
		...
		writel(base_low,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr +
				       C_HMC_TX_WINDOW_RDMAIO_BASE_LOW));
		writel(base_high,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr +
				       C_HMC_TX_WINDOW_RDMAIO_BASE_HIGH));
		break;
	case ZXDH_HMC_IW_QP:
		val = zxdh_hmc_register_config_comval(dev, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_QPC_RX));

		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_QPC_RX_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_QPC_RX_BASE_HIGH));
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_QPC_TX));

		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_QPC_TX_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_QPC_TX_BASE_HIGH));
		break;
	case ZXDH_HMC_IW_SRQ:
		val = zxdh_hmc_register_config_comval(dev, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_SRQC_RX));
		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_SRQC_RX_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_SRQC_RX_BASE_HIGH));
		break;
	case ZXDH_HMC_IW_CQ:
		val = zxdh_hmc_register_config_comval(dev, rsrc_type);
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_CQC_RX1));
		writel(val,
		       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_HMC_CQC_RX2));
		writel(base_low, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						 C_HMC_CQC_RX_BASE_LOW));
		writel(base_high, (u32 __iomem *)(cqp->dev->hw->hw_addr +
						  C_HMC_CQC_RX_BASE_HIGH));
		break;
	default:
		break;
	}
