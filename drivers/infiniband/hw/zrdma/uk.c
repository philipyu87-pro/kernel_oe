	...
	if (info->error) {
		...
		if (info->major_err == ZXDH_FLUSH_MAJOR_ERR) {
			...
			if (info->minor_err != FLUSH_GENERAL_ERR) {
				...
				qword0 |= FIELD_PREP(ZXDH_CQ_MINERR,
						     FLUSH_GENERAL_ERR);
	...
	if (q_type == ZXDH_CQE_QTYPE_RQ) {
		...
		if (info->comp_status == ZXDH_COMPL_STATUS_FLUSHED ||
			...
			if (qp->is_srq == false) {
				...
				info->wr_id =
					qp->rq_wrid_array[qp->rq_ring.tail];
		...
		if (info->imm_valid) {
			info->imm_data =
				(u32)FIELD_GET(ZXDH_CQ_IMMDATA, qword3);
		}
		...
		if (qp_type == ZXDH_QP_TYPE_ROCE_RC) {
			...
			info->ud_src_qpn =
				(u32)FIELD_GET(IRDMACQ_UDSRCQPN, qword2);
		...
		if (info->comp_status != ZXDH_COMPL_STATUS_FLUSHED) {
			...
			do {
				...
				op_type = (u8)FIELD_GET(IRDMAQPSQ_OPCODE,
							wqe_qword);
