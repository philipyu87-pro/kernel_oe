...
#define ZXDH_SMMU_PT_TOTAL_SIZE		(ZXDH_SMMU_L1_PT_SIZE + \
					 ZXDH_SMMU_L2_PT_SIZE + \
					 ZXDH_SMMU_L3_PT_SIZE)
...
static int zxdh_smmu_alloc_cma_memory(struct zxdh_sc_dev *dev)
{
	...
	pte_addr->cma_page_addr = dma_alloc_coherent(device,
						     ZXDH_SMMU_PT_TOTAL_SIZE,
...
int zxdh_smmu_pagetable_exit(struct zxdh_sc_dev *dev)
{
	...
	/* Free CMA memory */
	if (pte_addr->cma_page_addr) {
		dma_free_coherent(device, ZXDH_SMMU_PT_TOTAL_SIZE,
				  pte_addr->cma_page_addr,
				  pte_addr->cma_page_mem_base_pa);
	}
	...
	if (pte_addr->pte_temp_vir_addr) {
		dma_free_coherent(device, ZXDH_SMMU_L1_ENTRY_SIZE * 4,
				  (void *)pte_addr->pte_temp_vir_addr,
				  pte_addr->pte_temp_phy_addr);
	}
...
/**
 * zxdh_smmu_invalidate_tlb - Invalidate SMMU TLB
 * @dev: Device context
 *
 * Send TLB invalidation command to SMMU hardware.
 *
 * Return: 0 on success, negative error code on failure
 */
int zxdh_smmu_invalidate_tlb(struct zxdh_sc_dev *dev)
{
	...
	in.payload_addr = (uint8_t *)&smmu_info;
