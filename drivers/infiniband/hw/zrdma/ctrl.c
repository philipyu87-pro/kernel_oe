			    ...
			    struct zxdh_qp_host_ctx_info *info)
{
	...
	if (qp->qp_uk.qp_type == ZXDH_QP_TYPE_ROCE_RC) {
		...
		gqp_id = zxdh_get_rc_gqp_id(qp->qp_uk.qp_8k_index,
	...
	hdr = FIELD_PREP(RDMAQPC_RX_WQE_SIGN_EN, 0) |
	      ...
	      FIELD_PREP(RDMAQPC_RX_IRD_SIZE,
			 zxdh_sc_get_encoded_ird_size(roce_info->ird_size)) |
...
int zxdh_sc_cqp_init(struct zxdh_sc_cqp *cqp, struct zxdh_cqp_init_info *info)
{
	...
	writel(ZXDH_CQPDB_INIT_VALUE,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_DB));
	writel(0, (u32 __iomem *)(cqp->dev->hw->hw_addr +
				  C_RDMA_CQP_MGC_BASE_HIGH));
	writel(0, (u32 __iomem *)(cqp->dev->hw->hw_addr +
				  C_RDMA_CQP_MGC_BASE_LOW));
	writel(0,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_AH_CACHE_ID));
	writel(0, (u32 __iomem *)(cqp->dev->hw->hw_addr +
				  C_RDMA_CQP_MGC_INDICATE_ID));
	writel(0, (u32 __iomem *)(cqp->dev->hw->hw_addr +
...
int zxdh_sc_cqp_create(struct zxdh_sc_cqp *cqp, u16 *maj_err, u16 *min_err)
{
	...
	//reset CQP status
	writel(0,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CONFIG_DONE));
	mdelay(5);
	...
	writel(temp, (u32 __iomem *)(cqp->dev->hw->hw_addr +
				     C_RDMA_CQP_PF_VF_ID(cqp->dev->vhca_id)));
	...
	writel(temp,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CONTEXT_0));
	// CQP_Context_1
	writel(cqp->dev->base_qpn,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CONTEXT_1));
	// CQP_Context_2
	temp = (u32)FIELD_GET(ZXDH_CQPADDR_HIGH, cqp->sq_pa);
	writel(temp,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CONTEXT_2));
	// CQP_Context_3
	temp = (u32)FIELD_GET(ZXDH_CQPADDR_LOW, cqp->sq_pa);
	writel(temp,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CONTEXT_3));
	// CQP_Context_4
	temp = (u32)FIELD_GET(ZXDH_CQPADDR_HIGH, (uintptr_t)cqp);
	writel(temp,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CONTEXT_4));
	// CQP_Context_5
	...
	writel(ZXDH_CCQN_INIT_VALUE,
	       (u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_CQ_NUM));
...
err:
	err_code = readl(
		(u32 __iomem *)(cqp->dev->hw->hw_addr + C_RDMA_CQP_ERRCODE));
...
int zxdh_sc_ccq_init(struct zxdh_sc_cq *cq, struct zxdh_ccq_init_info *info)
{
	...
	writel(cq->cq_uk.cq_id,
	       (u32 __iomem *)(cq->dev->hw->hw_addr + C_RDMA_CQP_CQ_NUM));
...
void zxdh_hmc_dpu_capability(struct zxdh_sc_dev *dev)
{
	...
	writel(val, (u32 __iomem *)(dev->hw->hw_addr +
				    RDMATX_ACK_PCI_MAX_MRTE_INDEX_PARA_CFG));
	writel(val, (u32 __iomem *)(dev->hw->hw_addr +
				    RDMARX_PCI_MAX_MRTE_INDEX_RAM));
	writel(val, (u32 __iomem *)(dev->hw->hw_addr +
				    RDMATX_LOCAL_MRTE_PARENT_PARA_CFG));
}
	...
	if(rf->sc_dev.flr_query == ZXDH_FLR_QUERY_FLAG) {
		do {
			...
			if (cnt++ > ZXDH_FLR_QUERY_CNT) {
				writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMA_CQP_CONTEXT_6));
...
u16 zxdh_get_tc_8k_index_offset(u32 total_vhca, u16 vhca_8k_index_cnt,
						u8 traffic_class, u16 *tc_8k_index_num)
{
...
u16 zxdh_get_8k_index(struct zxdh_sc_qp *qp, u32 dest_ip)
{
	...
	tc_8k_index_offset = zxdh_get_tc_8k_index_offset(qp->dev->total_vhca, 
					qp->dev->vhca_8k_index_cnt, qp->user_pri, &tc_8k_index_num);
	dip_8k_index_offset = dest_ip % tc_8k_index_num;
	qp_8k_index = qp->dev->vhca_8k_index_start +
					tc_8k_index_offset + dip_8k_index_offset;
	return qp_8k_index;
}
