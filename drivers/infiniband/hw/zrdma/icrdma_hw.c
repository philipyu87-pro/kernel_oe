...
static void zxdh_init_ceq_hw(struct zxdh_sc_dev *dev)
{
	...
	dev->ceq_axi.ceqe_axi_info =
		(u32 __iomem *)(hw_addr + C_CEQ_CEQE_AXI_INFO_RAM);
	dev->ceq_axi.rpble_axi_info =
		(u32 __iomem *)(hw_addr + C_CEQ_RPBLE_AXI_INFO_RAM);
	dev->ceq_axi.lpble_axi_info =
		(u32 __iomem *)(hw_addr + C_CEQ_LPBLE_AXI_INFO_RAM);
...
static void zxdh_init_aeq_hw(struct zxdh_sc_dev *dev)
{
	...
	dev->aeq_tail_pointer =
		(u32 __iomem *)(hw_addr + C_RDMA_CPU_SOFTWARE_TAIL);
	dev->aeq_vhca_pfvf.aeq_msix_data =
		(u32 __iomem *)(hw_addr + RDMA_CPU_MSIX_DATA);
	dev->aeq_vhca_pfvf.aeq_msix_config =
		(u32 __iomem *)(hw_addr + RDMA_CPU_MSIX_CONFIG);
	dev->aeq_vhca_pfvf.aeq_root_axi_data =
		(u32 __iomem *)(hw_addr + AEQ_REPORT_ROOT_AXI_DATA);
	dev->aeq_vhca_pfvf.aeq_leaf_axi_data =
		(u32 __iomem *)(hw_addr + AEQ_REPORT_LEAF_AXI_DATA);
	dev->aeq_vhca_pfvf.aeq_wr_axi_data =
		(u32 __iomem *)(hw_addr + AEQ_REPORT_WR_AXI_DATA);
	dev->aeq_vhca_pfvf.aeq_aee_flag =
		(u32 __iomem *)(hw_addr + AEQ_AEQC_AEE_FLAG);
...
void zxdh_init_hw(struct zxdh_sc_dev *dev)
{
	...
	writel(IRDMARX_RD_TIME_LIMIT_VALUE,
	       (u32 __iomem *)(hw_addr + RDMATX_RD_TIME_LIMIT));
	writel(IRDMARX_RD_TIME_LIMIT_VALUE,
	       (u32 __iomem *)(hw_addr + RDMARX_RD_TIME_LIMIT));
}
