				...
				u32 ram_width, u32 ram_addr, u32 ram_read_cnt)
{
	...
	writel(ram_read_cnt,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMARX_RAM_READ_LENGTH));
...
static int zxdh_read_rx_ram_flag(struct zxdh_sc_dev *dev)
{
	...
	val |= readl(
		(u32 __iomem *)(dev->hw->hw_addr + RDMARX_DOUBLE_VLD_FLAG));
	if (val != 0)
		return -EIO;
...
static u32 zxdh_read_rx_ram_data(struct zxdh_sc_dev *dev, u32 offset_idx)
{
	...
	val = readl((u32 __iomem *)(dev->hw->hw_addr +
				    RDMARX_RAM_MAINTENANCE_RAM(offset_idx)));
	return val;
}
				...
				u32 ram_width, u32 ram_addr, u32 ram_read_cnt)
{
	...
	writel(ram_read_cnt,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_RAM_READ_LENGTH));
...
static int zxdh_read_tx_ram_flag(struct zxdh_sc_dev *dev)
{
	...
	val |= readl(
		(u32 __iomem *)(dev->hw->hw_addr + RDMATX_DOUBLE_VLD_FLAG));
	if (val != 0)
		return -EIO;
	return val;
}
...
static u32 zxdh_read_tx_ram_data(struct zxdh_sc_dev *dev, u32 offset_idx)
{
	...
	val = readl((u32 __iomem *)(dev->hw->hw_addr +
				    RDMATX_RAM_MAINTENANCE_RAM(offset_idx)));
	return val;
}
				...
				struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		check_ram_num = readl(
			(u32 __iomem *)(dev->hw->hw_addr + RDMARX_RAM_NUM));
		check_ram_addr = readl(
			(u32 __iomem *)(dev->hw->hw_addr + RDMARX_RAM_ADDR));
				...
				struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		check_ram_num = readl(
			(u32 __iomem *)(dev->hw->hw_addr + RDMATX_RAM_NUM));
		check_ram_addr = readl(
			(u32 __iomem *)(dev->hw->hw_addr + RDMATX_RAM_ADDR));
				...
				struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		rdma_stats
			->rdma_stats_entry[HW_STAT_REQ_REMOTE_INVALID_REQUEST] =
			val;
		rdma_stats->rdma_stats_entry_sta
			[HW_STAT_REQ_REMOTE_INVALID_REQUEST] =
			ZXDH_HW_STATS_VALID;

		val = zxdh_read_tx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_2);
		rdma_stats->rdma_stats_entry[HW_STAT_REQ_REMOTE_ACCESS_ERRORS] =
			val;
		rdma_stats
			->rdma_stats_entry_sta[HW_STAT_REQ_REMOTE_ACCESS_ERRORS] =
			ZXDH_HW_STATS_VALID;

		val = zxdh_read_tx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_3);
		rdma_stats
			->rdma_stats_entry[HW_STAT_REQ_REMOTE_OPERATION_ERRORS] =
			val;
		rdma_stats->rdma_stats_entry_sta
				...
				struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		val = zxdh_read_tx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_1);
		rdma_stats->rdma_stats_entry[HW_STAT_REQ_LOCAL_LENGTH_ERROR] =
			val;
		rdma_stats
			->rdma_stats_entry_sta[HW_STAT_REQ_LOCAL_LENGTH_ERROR] =
			ZXDH_HW_STATS_VALID;
		return 0;
	}
	pr_err("%s: get ram data failed !\n", __func__);
				...
				struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		val = zxdh_read_rx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_0);
		rdma_stats->rdma_stats_entry[HW_STAT_ROCE_SLOW_RESTART_CNPS] =
			val;
		rdma_stats
			->rdma_stats_entry_sta[HW_STAT_ROCE_SLOW_RESTART_CNPS] =
			ZXDH_HW_STATS_VALID;
		return 0;
	}
	pr_err("%s: get ram data failed !\n", __func__);
				 ...
				 struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		rdma_stats
			->rdma_stats_entry[HW_STAT_RESP_REMOTE_INVALID_REQUEST] =
			val;
		rdma_stats->rdma_stats_entry_sta
		...
		val = zxdh_read_rx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_3);
		rdma_stats->rdma_stats_entry[HW_STAT_RESP_REMOTE_ACCESS_ERRORS] =
			val;
		rdma_stats->rdma_stats_entry_sta
				 ...
				 struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		val = zxdh_read_rx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_0);
		rdma_stats
			->rdma_stats_entry[HW_STAT_RESP_REMOTE_OPERATION_ERRORS] =
			val;
		rdma_stats->rdma_stats_entry_sta
				 ...
				 struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		val = zxdh_read_rx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_0);
		rdma_stats
			->rdma_stats_entry[HW_STAT_NP_ECN_MARKED_ROCE_PACKETS] =
			val;
		rdma_stats->rdma_stats_entry_sta
				 ...
				 struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	for (i = 0; i < ZXDH_RAM_REPEAT_READ_CNT; i++) {
		...
		temp_val = zxdh_read_rx_ram_data(dev, ZXDH_RAM_32_BIT_IDX_0);
		temp_val = ((temp_val & ZXDH_32_BIT_MASK_16_31) >>
			    IRMDA_BIT_WIDTH_16);
		if (val >= temp_val)
			val = val - temp_val;
				 ...
				 struct zxdh_rdma_stats_get *rdma_stats)
{
	...
	pr_info("%s dev->vhca_id:%d vport:0x%x tx_pkts:%llu tx_bytes:%llu\n",
		__func__, dev->vhca_id, vport, tx_pkts, tx_bytes);
}
...
int zxdh_generated_cmpls(struct zxdh_cq *iwcq,
...
int get_pci_board_bdf(char *pci_board_bdf, struct zxdh_pci_f *rf)
{
	...
	if (rf->pcidev == NULL || rf->pcidev->bus == NULL) {
		pr_info("get_pci_board_bdf fail:rf pcidev is null\n");
		return -EIO;
	}
