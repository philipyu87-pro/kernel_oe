...
int zxdh_vf_init_np_tbl(struct zxdh_pci_f *rf)
{
	...
	if (!rf->sc_dev.np_mode_low_lat) {
		...
		ether_addr_copy(rf->iwdev->mac_addr,
...
void zxdh_aeq_process_stop_cap(struct zxdh_pci_f *rf)
{
	...
	if (!stop_cap_work) {
		pr_err("kzalloc stop_cap_work failed!\n");
		return;
	}
...
void zrdma_cleanup_rdma_tools_cfg(struct zxdh_pci_f *rf)
{
	...
	if (rf->sc_dev.hw_attrs.self_health == false) {
		if (zxdh_rdma_reg_write(rf, RDMATX_DATA_START_CAP, 0))
			pr_err("zrdma_cleanup_rdma_tools_cfg write RDMATX_DATA_START_CAP failed\n");
		if (zxdh_rdma_reg_write(rf, RDMARX_DATA_START_CAP, 0))
			pr_err("zrdma_cleanup_rdma_tools_cfg write RDMARX_DATA_START_CAP failed\n");
	}
...
static void zxdh_shutdown_vhca(struct zxdh_pci_f *rf)
{
	...
	qpc_axi_info |= (3 << 2);
	writel(qpc_axi_info,
		       (u32 __iomem *)(rf->sc_dev.hw->hw_addr + C_HMC_QPC_RX));
}
	...
	if (rf->ftype == 0) {
		for (vf_id = 0; vf_id < rf->max_rdma_vfs; vf_id++) {
			...
			if (vf_dev) {
				...
				zxdh_put_vfdev(&rf->sc_dev,
					       rf->sc_dev.vf_dev[vf_id]);
				zxdh_remove_vf_dev(&rf->sc_dev,
						   rf->sc_dev.vf_dev[vf_id]);
			}
		}
	}
...
int zxdh_rt_init_hw(struct zxdh_device *iwdev)
{
	...
	do {
		...
		iwdev->cleanup_wq = alloc_workqueue(
			"zrdma-cleanup-wq", WQ_UNBOUND, WQ_UNBOUND_MAX_ACTIVE);
		if (!iwdev->cleanup_wq)
			return -ENOMEM;
...
static void zxdh_config_tx_regs(struct zxdh_sc_dev *dev)
{
	...
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_ACK_SQWQE_PARA_CFG));
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_ACK_DDR_PARA_CFG));
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_DB_SQWQE_ID_CFG));
	writel(temp, (u32 __iomem *)(dev->hw->hw_addr + RDMATX_SQWQE_PARA_CFG));
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_PAYLOAD_PARA_CFG));
	...
	writel(0x1,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_ADD_TOKEN_CHANGE_EN));
	writel(0x1900,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_TIME_ADD_TOKEN_CFG));
	writel(0x132d7,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_SIZE_ADD_TOKEN_CFG));
	writel(0x3FFFFFF,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMATX_TOKEN_MAX_CFG));
}
...
static void zxdh_config_rx_regs(struct zxdh_sc_dev *dev)
{
	...
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMARX_ACK_RQDB_AXI_RAM));
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMARX_CQ_CQE_AXI_INFO_RAM));
	writel(temp,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMARX_CQ_DBSA_AXI_INFO_RAM));
	writel(dev->hmc_fn_id, (u32 __iomem *)(dev->hw->hw_addr +
					       RDMARX_MUL_CACHE_CFG_SIDN_RAM));
	writel((ZXDH_AXID_HOST_EP0 + dev->ep_id),
	       (u32 __iomem *)(dev->hw->hw_addr +
			       RDMARX_MUL_COPY_QPN_INDICATE));
	writel(RDMARX_MAX_MSG_SIZE,
	       (u32 __iomem *)(dev->hw->hw_addr + RDMARX_VHCA_MAX_SIZE_RAM));
...
static void zxdh_config_io_regs(struct zxdh_sc_dev *dev)
{
	...
	for (temp0 = 0; temp0 < 32; temp0++) {
		if (temp0 < ZXDH_RW_PAYLOAD || temp0 == ZXDH_QPC_OBJ_ID) {
			...
			writel((rf->ftype), (u32 __iomem *)(dev->hw->hw_addr +
							    C_RDMAIO_TABLE5_0 +
							    (temp0 * 4)));
		}
	}
	...
	if (rf->ftype == 0) {
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_0));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_1));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_2));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_3));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_4));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_5));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_6));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_7));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_8));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_9));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_10));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_11));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_12));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_13));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_14));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE6_15));

		temp2 = FIELD_PREP(ZXDH_IOTABLE7_PFID, rf->pf_id) |
			FIELD_PREP(ZXDH_IOTABLE7_EPID,
				   (ZXDH_HOST_EP0_ID + rf->ep_id));
		writel(temp2,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE7));
	} else {
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_0));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_1));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_2));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_3));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_4));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_5));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_6));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_7));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_8));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_9));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_10));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_11));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_12));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_13));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_14));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_15));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_16));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_17));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_18));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_19));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_20));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_21));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_22));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_23));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_24));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_25));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_26));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_27));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_28));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_29));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_30));
		writel(0,
		       (u32 __iomem *)(dev->hw->hw_addr + C_RDMAIO_TABLE5_31));
	}
}
...
static void zxdh_set_hw_rsrc(struct zxdh_pci_f *rf)
{
	...
	rf->qp_table =
		(struct zxdh_qp *
			 *)(&rf->allocated_mcgs[BITS_TO_LONGS(rf->max_mcg)]);
#endif
