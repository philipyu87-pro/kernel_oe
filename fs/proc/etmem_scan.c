		...
		struct mm_walk *walk)
{
	...
	/*
	 * This host fallback walk runs after dropping kvm->mmu_lock, and
	 * mmap_lock serializes the host page table traversal itself. Keep IRQs
	 * enabled here so large fallback walks do not trigger soft lockups.
	 */
